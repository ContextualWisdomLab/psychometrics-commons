...
#[test]
fn identity_aliases_numeric_ids_and_blank_references_fail_closed() {
    for invalid in ["", " ", " audit_event_alias ", "12345", "+12.3e4"] {
...
fn purpose_and_action_codes_are_stable_lowercase_ascii_tokens() {
    for invalid_code in [
        ...
        assert_eq!(AuditEvidence::new(invalid).unwrap_err(), AuditEvidenceError::InvalidCode);
...
fn digest_and_server_time_fail_closed() {
    for invalid_digest in [
        ...
        assert_eq!(AuditEvidence::new(invalid).unwrap_err(), AuditEvidenceError::InvalidDigest);
    }

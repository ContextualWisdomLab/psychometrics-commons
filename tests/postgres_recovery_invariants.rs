...
//!
//! After migration `0019`, a `processing` row must carry `claim_deadline_at`. The deadline
//! trigger is `BEFORE UPDATE` only, so this fixture seeds the wall-clock explicitly instead of
//! expecting an insert trigger to invent it. Restore must keep that exact timestamp so a
//! recovered worker cannot gain a fresh lease.

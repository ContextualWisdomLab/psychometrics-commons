...
//!
//! An opaque reference is a product-issued identifier whose accepted spelling identifies exactly one
//! resource. A Unicode scalar value is a valid Unicode code point representable by Rust `char`;
//! surrogate code points are excluded. A default-ignorable character is a Unicode formatting or
//! normally invisible character that the product rejects so visually identical spellings cannot
//! become different identities. The checks below cover control characters, boundary whitespace,
//! numeric characters, and default-ignorable characters across the representable Unicode range.
...
#[test]
fn postgres_boundary_whitespace_matches_rust_is_whitespace_in_both_directions() {
    let (mut client, schema) = test_client();
    let mut samples = Vec::with_capacity(PARITY_BATCH_SIZE);
    let mut expected = Vec::with_capacity(PARITY_BATCH_SIZE);

    // Put every otherwise-safe scalar at both reference boundaries. Rust trims exactly the
    // characters for which `is_whitespace` is true, so durable storage must reject those padded
    // spellings and preserve non-whitespace characters unchanged.
    for scalar in (1u32..=0x0010_FFFF).filter_map(char::from_u32) {
        if scalar.is_control() || is_default_ignorable(scalar) {
            continue;
        }

        for sample in [format!("{scalar}ab"), format!("ab{scalar}")] {
            samples.push(sample);
            expected.push(!scalar.is_whitespace());

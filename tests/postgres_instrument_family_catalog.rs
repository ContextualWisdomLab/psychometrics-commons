...
//! bounded deterministic query whose rows are revalidated as Published snapshots.
...
use psychometrics_commons_runtime::postgres_instrument_catalog::{
    list_startable_instrument_releases_for_family, StartableInstrumentCatalogError,
...
const DATABASE_TEST_LOCK_KEY: i64 = 0x4946_4341_5441_4c47;
...
#[test]
fn family_catalog_fails_closed_when_one_family_exceeds_a_bounded_page() {
    let _guard = test_guard();
    let mut client = test_client();
    reset_tables(&mut client);
    apply_instrument_release_migration(&mut client).unwrap();
    ...
    assert!(matches!(
        list_startable_instrument_releases_for_family(&mut transaction, "instrument_big_five"),
        Err(StartableInstrumentCatalogError::PageRequired)
    ));
    transaction.commit().unwrap();
}

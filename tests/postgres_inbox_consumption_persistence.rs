...
fn stored_claim_expiry(client: &mut Client, consumption_ref: &str) -> Option<i64> {
    client
        .query_one(
            "SELECT claim_expires_at_unix_ms FROM integration_consumption \
             WHERE consumption_ref = $1",
            &[&consumption_ref],
        )
        .unwrap()
        .get(0)
...
fn local_complete_and_claimed_complete_are_fenced() {
    ...
    assert_eq!(stored_claim_expiry(&mut client, "consumption_local"), None);
    ...
    assert_eq!(
        stored_claim_expiry(&mut client, "consumption_claimed"),
        None
    );
}
...
fn quarantine_and_terminal_transitions_fail_closed() {
    ...
    assert_eq!(
        stored_claim_expiry(&mut client, "consumption_quarantine"),
        None
    );
...
fn isolation_and_overflow_persist_fail_closed() {
    ...
    let mut transaction = client.transaction().unwrap();
    assert!(matches!(
        begin_inbox_consumption(&mut transaction, &consumption, 20_001, too_large),
        Err(InboxConsumptionPersistenceError::TimestampOutOfRange)
    ));
    assert!(matches!(
        expire_inbox_consumption(&mut transaction, &consumption, too_large),
        Err(InboxConsumptionPersistenceError::TimestampOutOfRange)
    ));
    assert!(matches!(
        complete_inbox_consumption(

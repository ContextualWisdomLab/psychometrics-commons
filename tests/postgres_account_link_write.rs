...
//! After unlink, a leftover current row must not tell the next buyer the
//! account is taken.
...
fn insert_stale_current_projection(client: &mut Client) {
    client
        .execute(
...
fn persist_unlinked_history(client: &mut Client) {
    let mut unlinked = anonymous_participant();
    unlinked
        .link_account(
            "link_event_identity_write",
            "keyverse_issuer_write",
    ...
    persist_participant_identity_history(&mut transaction, &unlinked).unwrap();
...
#[test]
fn write_command_rebinds_freed_subject_after_stale_current() {
    let _guard = write_test_guard();
    let mut client = test_client();
    reset_tables(&mut client);
    apply_participant_identity_link_migration(&mut client).unwrap();
    persist_unlinked_history(&mut client);
    insert_stale_current_projection(&mut client);
    ...
    .unwrap();
    let next_anonymous = AnonymousSessionContext::new(
        "tenant_identity_write",
        "participant_identity_write_beta",
        "session_identity_write_beta",
        "anonymous_proof_write_beta",
    ...
    assert_eq!(
        recovered.link_event_ref(),
        Some("link_event_identity_write_beta")
    );
}

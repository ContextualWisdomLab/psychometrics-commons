...
use psychometrics_commons_runtime::postgres_response::ResponsePersistenceError;
...
fn persistence_errors_expose_stable_messages_and_database_sources() {
    for (error, expected_message) in [
        (
            ...
            "response persistence references must be opaque values",
        ),
        (
        ...
        (
            ResponsePersistenceError::InvalidSequence,
            "response event sequence exceeds the PostgreSQL bigint range",
        ),
        (
            ResponsePersistenceError::UnsupportedIsolationLevel,
    ...
    let database_error = client
        ...
        .unwrap_err();
    let error = ResponsePersistenceError::from(database_error);
    assert_eq!(
        error.to_string(),

...
fn concurrent_exact_replay_observes_committed_winner_under_read_committed() {
    ...
    let replay_result = replay_thread.join().unwrap();
    assert_eq!(
        replay_result.unwrap(),
        AuditPersistenceDisposition::Duplicate,
        "a committed concurrent exact replay must remain idempotent instead of disappearing from the command snapshot"
    );
}

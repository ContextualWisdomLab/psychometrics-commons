...
#[test]
fn persist_query_arms_fail_closed_when_a_required_relation_is_missing() {
    let (_database_test_guard, mut client) = test_client();
    ...
    let mismatched = created_session(
        "ses_start_digest_mismatch_persist",
        PARTICIPANT_REF,
        "release_big_five_ko_v1",
        OTHER_DIGEST,
    );
    ...
    client
        .batch_execute("DROP TABLE IF EXISTS assessment_session_command; DROP TABLE IF EXISTS assessment_session;")
        .unwrap();
    let first = created_session(
        "ses_missing_session_table",
        PARTICIPANT_REF,
        "release_big_five_ko_v1",
        VALID_DIGEST,
    );
    let mut transaction = client.transaction().unwrap();
    assert!(matches!(
        persist_assessment_session(&mut transaction, &first),
    ...
    transaction.commit().unwrap();
    client
        .batch_execute("DROP TABLE IF EXISTS assessment_session_command;")
        .unwrap();
    let mut transaction = client.transaction().unwrap();
    assert!(matches!(
        persist_assessment_session_commands(&mut transaction, &created),
        Err(AssessmentSessionPersistenceError::Database(_))
    ));
    let mut activated = created.clone();
    activated
        .apply_command("cmd_activate_missing_table", 1, SessionCommand::Activate)
        .unwrap();
    assert!(matches!(
...
fn command_persist_locks_session_header_until_caller_commits() {
    ...
    let mut create_transaction = holder.transaction().unwrap();
    persist_assessment_session(&mut create_transaction, &created).unwrap();
    create_transaction.commit().unwrap();
    ...
    let lock_timeout_message = match &error {
        AssessmentSessionPersistenceError::Database(database_error) => database_error
            .as_db_error()
...
#[test]
fn command_persist_rejects_header_release_version_digest_locale_and_clock_rebinds() {
    let (_database_test_guard, mut client) = test_client();
    prepare_created_session_store(&mut client);
    let session = created_session(
        "ses_header_rebind_alpha",
        PARTICIPANT_REF,
        "release_big_five_ko_v1",
        VALID_DIGEST,
    );
    ...
    for (column, value) in [
        ...
        assert!(
            ...
            "{column} header rebind must fail closed"
        );
        transaction.rollback().unwrap();
        client
            .execute(
                "UPDATE assessment_session
                 SET instrument_release_ref = $2,
                     instrument_version_ref = $3,
                     instrument_release_content_digest = $4,
                     locale = $5
                 WHERE session_ref = $1",
                &[
                    &"ses_header_rebind_alpha",
    ...
    client
        .execute(
            "UPDATE assessment_session SET created_at_unix_ms = $2 WHERE session_ref = $1",
            &[&"ses_header_rebind_alpha", &20_001_i64],
        )
        .unwrap();
    let mut transaction = client.transaction().unwrap();
    assert!(matches!(
...
#[test]
fn command_persist_rejects_replayed_sequence_and_command_name_rebinds() {
    let (_database_test_guard, mut client) = test_client();
    prepare_created_session_store(&mut client);
    let mut session = created_session(
        "ses_command_sequence_alpha",
        PARTICIPANT_REF,
        "release_big_five_ko_v1",
        VALID_DIGEST,
    );
    ...
    persist_assessment_session(&mut transaction, &session).unwrap();
    session
        .apply_command("cmd_activate_sequence", 1, SessionCommand::Activate)
        .unwrap();
    persist_assessment_session_commands(&mut transaction, &session).unwrap();
    transaction.commit().unwrap();
    ...
    client
        .execute(
            "UPDATE assessment_session_command
             SET command_sequence = $2, command_name = $3
             WHERE session_ref = $1 AND command_ref = $4",
            &[
                &"ses_command_sequence_alpha",
                &1_i64,
                &"pause",

//! Fail-closed encoding preflight for durable response-snapshot reference parity.
//!
...
fn migration_checks_utf8_before_installing_unicode_reference_validator() {
    let migration = include_str!("../migrations/0010_response_snapshot.sql");
    ...
    let validator = migration
        .find("CREATE OR REPLACE FUNCTION response_snapshot_reference_is_valid")
        .expect("migration must install the response-snapshot reference validator");
    ...
    assert!(migration
        .contains("response_snapshot reference parity requires PostgreSQL server_encoding UTF8"));
}

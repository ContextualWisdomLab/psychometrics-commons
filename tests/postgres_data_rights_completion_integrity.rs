...
#[test]
fn completion_evidence_rejects_unicode_whitespace_aliases() {
    let mut client = client("data_rights_completion_unicode_evidence");
    let request = persist_processing(
        &mut client,
        "data_rights_request_unicode_evidence",
        DataRightsRequestKind::Deletion,
    );

    for invalid_evidence in ["\tcompletion_evidence_direct", "\u{00a0}completion_evidence_direct"] {
        assert!(
            client
                .execute(
                    "UPDATE data_rights_request_state
                     SET current_state = 'completed',
                         completion_evidence_ref = $1,
                         completed_at_unix_ms = 10300,
                         latest_event_at_unix_ms = 10300
                     WHERE request_ref = $2",
                    &[&invalid_evidence, &request.request_ref()],
                )
                .is_err(),
            "database must reject completion evidence spelling rejected by the Rust domain"
        );
    }
}

#[test]
fn retained_scope_rejects_unicode_whitespace_aliases() {
    let mut client = client("data_rights_completion_unicode_retained_scope");
    let mut request = persist_processing(
        &mut client,
        "data_rights_request_unicode_retained_scope",
        DataRightsRequestKind::Deletion,
    );
    request
        .complete("completion_evidence_retained_scope", &["retention_legal"], 10_300)
        .unwrap();
    {
        let mut transaction = client.transaction().unwrap();
        persist_data_rights_completion(&mut transaction, &request).unwrap();
    ...
    for invalid_scope in ["\tretention_additional", "\u{00a0}retention_additional"] {
        assert!(
            client
                .execute(
                    "INSERT INTO data_rights_retained_scope_evidence
                        (request_ref, tenant_ref, retained_scope_ref)
                     VALUES ($1, $2, $3)",
                    &[&request.request_ref(), &request.tenant_ref(), &invalid_scope],
                )
                .is_err(),
            "database must reject retained-scope spelling rejected by the Rust domain"
        );
    }
}

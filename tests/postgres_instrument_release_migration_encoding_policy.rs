//! Fail-closed encoding preflight for immutable instrument-release reference parity.
//!
...
fn migration_checks_utf8_before_installing_unicode_reference_validator() {
    let migration = include_str!("../migrations/0006_instrument_release.sql");
    ...
    let validator = migration
        .find("CREATE OR REPLACE FUNCTION instrument_release_reference_is_valid")
        .expect("migration must install the instrument-release reference validator");
    ...
    assert!(migration
        .contains("instrument_release reference parity requires PostgreSQL server_encoding UTF8"));
}

...
//! standard error is present, the score is distinguishable from zero at a conventional 95%
//! interval (`|score| >= 1.96 * SE`). Profiles with no expressed dimension receive
//! `style_balanced_profile` instead of a forced category.
//!
//! The style names are original product presentation labels. They are not official type scores
//! and must not be advertised as psychometric latent classes. Continuous scores remain the
//! measurement source. Narrative text is separately versioned so this mapping cannot hide
//! uncertainty behind generic personality copy. Authoritative citations live in
//! `docs/adr/0018-continuous-scores-and-narrative-separation.md` (APA 7th).
...
/// and never changes numeric score values.
///
/// # Errors
///

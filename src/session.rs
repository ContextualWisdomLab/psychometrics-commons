...
impl AssessmentSession {
    ...
    ///
    /// Use this after [`crate::postgres_instrument_release::load_published_instrument_release`].
    /// It does not re-check publication lifecycle; the load boundary is the eligibility gate.
    /// Call [`AssessmentSession::new`] when the caller still holds a live
    /// [`InstrumentRelease`].
    ///
    /// # Errors
    ///
    /// Returns [`SessionCreationError::InvalidReference`] for malformed session/participant
    /// references, [`SessionCreationError::InvalidTimestamp`] for a zero server timestamp,
    /// or [`SessionCreationError::LocaleMismatch`] when the requested locale is not exactly
    /// the manifest locale.

...
///
/// Call this when a product flow already knows both the tenant and the audit-event identity. The
/// function returns the stored record when that pair matches, or `None` when the event is absent
/// for that tenant. A missing result intentionally does not reveal whether the same event exists
/// under a different tenant.
///
/// Stored fields are rebuilt through the current domain constructor. If a stored row no longer
/// satisfies that contract, the function fails instead of returning a weakened record. This
/// single-row read is valid at `READ COMMITTED` and stronger isolation levels because it does not
/// depend on a later command seeing a concurrent unique-key winner.
///
/// # Errors
///
...
/// Compare the stored row with caller evidence after insert or unique-key collision.
///
/// # Errors
///
...
pub fn insert_audit_row(
    ...
    transaction
        ...
        .map_err(AuditPersistenceError::from)
}
...
mod tests {
    ...
    fn caller_aliases_fail_closed_before_database_access() {
        for invalid in ["", " ", " audit_event_alias ", "123", "audit\u{200b}_event"] {
            assert!(matches!(

...
/// calling the scoring engine again. Do not use it to invent a new score; that
/// path remains [`ResultSnapshot::new`].
...
impl ResultSnapshot {
    ...
    ///
    /// Stored identities, consent, provenance, and copied observations must be
    /// canonical values that [`Self::new`] could have accepted at publication.
    /// The rebuilt snapshot can be served or compared for exact persist replay.
    /// This path never recomputes psychometric scores.
    ///
    /// # Errors
    ///
    /// Returns [`ResultSnapshotError::EmptyReference`] for a blank or
    /// non-canonical stored identity, [`ResultSnapshotError::MissingConsentSnapshot`]
    /// when no consent evidence is stored,
    /// [`ResultSnapshotError::DuplicateConsentSnapshot`] when a consent reference
    /// repeats, [`ResultSnapshotError::InvalidCreationTime`] when creation time is
    /// zero, [`ResultSnapshotError::SelfSupersession`] when the snapshot names
    /// itself as its predecessor, or [`ResultSnapshotError::InconsistentEvidence`]
    /// when the digest, schema, or observation set cannot reconstruct a published
    /// result.
    pub fn from_durable_evidence(
        evidence: ResultSnapshotEvidence<'_>,
    ) -> Result<Self, ResultSnapshotError> {
        let snapshot_ref = required_stored_reference(evidence.result_snapshot_ref)?;
        let participant_ref = required_stored_reference(evidence.participant_ref)?;
        let scoring_result_ref = required_stored_reference(evidence.scoring_result_ref)?;
        let session_ref = required_stored_reference(evidence.session_ref)?;
        let response_snapshot_ref = required_stored_reference(evidence.response_snapshot_ref)?;
        let assessment_spec_ref = required_stored_reference(evidence.assessment_spec_ref)?;
        let instrument_version_ref = required_stored_reference(evidence.instrument_version_ref)?;
        let scoring_version_ref = required_stored_reference(evidence.scoring_version_ref)?;
        let calibration_reference = required_stored_reference(evidence.calibration_reference)?;
        let narrative_version_ref = required_stored_reference(evidence.narrative_version_ref)?;
        let norm_version_ref = evidence
            .norm_version_ref
            .map(required_stored_reference)
            .transpose()?;

        if !is_canonical_sha256(evidence.engine_artifact_digest)
            || evidence.requested_output_schema_version != 1
            || evidence.score_observations.is_empty()
        {
        ...
        if evidence.score_observations.iter().any(|observation| {
            required_stored_reference(observation.construct_ref()).is_err()
                || !constructs.insert(observation.construct_ref().to_owned())
        }) {
            return Err(ResultSnapshotError::InconsistentEvidence);
        }
        ...
        for consent_ref in evidence.consent_snapshot_refs {
            let consent_ref = required_stored_reference(consent_ref)?;
            if !consent_refs.insert(consent_ref.to_owned()) {
                return Err(ResultSnapshotError::DuplicateConsentSnapshot);
            }
            normalized_consents.push(consent_ref.to_owned());
...
pub enum ResultSnapshotError {
    /// A required or supplied optional reference is blank or non-canonical.
    EmptyReference,
...
fn required_stored_reference(reference: &str) -> Result<&str, ResultSnapshotError> {
    match normalized_reference(reference) {
        Some(normalized) if normalized == reference => Ok(normalized),
        _ => Err(ResultSnapshotError::EmptyReference),
    }
}

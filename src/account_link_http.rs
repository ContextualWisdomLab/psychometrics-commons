//! Public HTTP transport for dual-proof account-link persist and recover.
//!
//! This slice exposes `POST /v1/account-links` and
//! `POST /v1/account-links/recover` over HTTP/1.1. Adapters validate
//! anonymous-session and Keyverse proofs before constructing the JSON body.
//! The handler does not parse tokens. It uses the server clock as
//! `linked_at_unix_ms` / recover `now`, then calls
//! [`crate::account_link_write::persist_authorized_account_link`] and
...
/// Server clock and process-local persist idempotency store.
...
impl AccountLinkHttpRuntime {
    ...
    ///
    /// `now_unix_ms` is the link and recover time. Callers must not pass a
    ...
    /// Return the server clock used for persist and recover.
    #[must_use]
...
/// Translate one raw HTTP/1.1 request into persist, recover, or a problem.
///
/// Unknown methods, paths, JSON shapes, and missing persist idempotency keys
/// fail closed with RFC 9457 problem details and do not open a transaction.
#[must_use]
pub fn classify_account_link_http_request(request: &str) -> AccountLinkHttpClassification {
    ...
    match (method, path) {
        ...
        (_, ACCOUNT_LINK_COLLECTION_PATH | ACCOUNT_LINK_RECOVER_PATH) => {
            AccountLinkHttpClassification::Ready(AccountLinkHttpResponse::problem(
                405,
                "urn:psychometrics-commons:problem:method-not-allowed",
                "Method Not Allowed",
                "account-link routes accept POST /v1/account-links and POST /v1/account-links/recover only",
            ))
        }
...
/// Authorize, persist or recover, and return the HTTP response.
///
/// Persist loads or mints the anonymous participant, then calls the dual-proof
/// write command with the server clock. Recover uses the same clock. If persist
/// fails after in-memory linking, the local participant is dropped.
#[must_use]
...
fn execute_persist(
    ...
    match persist_authorized_account_link(
        ...
        Ok(disposition) => {
            drop(participant);
            let recovered = match recover_participant_for_authenticated_account(
                transaction,
                &authenticated,
                runtime.now_unix_ms,
            ) {
                ...
                Ok(None) => {
                    return AccountLinkHttpResponse::problem(
                        ...
                        "persisted account link could not be recovered in the same transaction",
                    );
                }
            ...
            let body = account_link_body(&recovered, disposition_label);
        ...
        Err(error) => {
            drop(participant);
            map_account_link_write_error(&error)
        }
    }
}
    ...
    match error {
        AccountLinkWriteError::Authorization(AccountLinkAuthorizationError::InvalidTimestamp) => {
            AccountLinkHttpResponse::problem(
                ...
                "Server Clock Error",
                "account-link persist and recover require a server clock greater than zero",
            )
        }
        ...
        AccountLinkWriteError::Authorization(
            ...
            "anonymous and authenticated proofs must belong to the same tenant and participant",
        ),
        AccountLinkWriteError::Authorization(
        ...
        AccountLinkWriteError::Authorization(AccountLinkAuthorizationError::Participant(
        ...
        AccountLinkWriteError::Persistence(
        ...
        AccountLinkWriteError::Persistence(
            ...
            "urn:psychometrics-commons:problem:account-link-store",
            "Account Link Store Error",
            "account-link persist or recover could not use the product store",
        ),
    }
}
...
mod tests {
    use super::{
        ...
        parse_persist_body, reason_phrase, valid_idempotency_key, AccountLinkHttpClassification,
        RequestReadProgress, ACCOUNT_LINK_HTTP_MAX_REQUEST_BYTES,
    };
    ...
    #[test]
    fn classify_covers_recover_body_and_nested_account_link_paths() {
        assert!(matches!(

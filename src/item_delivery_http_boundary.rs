//! Hardened HTTP/1.1 framing for public item-delivery operations.
//!
//! `item_delivery_http.rs` owns application behavior. This boundary accepts one
//! request per connection, rejects `Transfer-Encoding`, duplicate framing or
//! idempotency headers, non-CRLF header lines, trailing bytes, and oversized or
//! incomplete frames before application dispatch. One overall read deadline
//! prevents slow-fragment clients from extending the connection indefinitely.

#[path = "item_delivery_http.rs"]
mod implementation;

pub use implementation::{
    bind_item_delivery_http, handle_item_delivery_http_request, ItemDeliveryHttpResponse,
    ItemDeliveryHttpRuntime, ItemDeliveryHttpRuntimeError, ITEM_DELIVERY_COLLECTION_SUFFIX,
    ITEM_DELIVERY_HTTP_IO_TIMEOUT, ITEM_DELIVERY_HTTP_MAX_REQUEST_BYTES,
};
...
/// Accept one connection and serve one item-delivery request.
///
/// # Errors
///
/// Returns [`io::ErrorKind::InvalidData`] for malformed, ambiguous, oversized,
/// incomplete, or multi-request framing; [`io::ErrorKind::TimedOut`] for an
/// expired overall request deadline; or the underlying socket I/O error.
    ...
    for line in headers.lines().skip(1) {
        ...
        if found.is_some() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "item-delivery HTTP request contains duplicate framing headers",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn framing_helpers_reject_ambiguous_or_unsupported_headers() {
        let plain = "GET /v1/sessions/s/item-deliveries HTTP/1.1\r\nHost: example.test\r\n\r\n";
        ...
        assert_eq!(single_header_value(duplicate, "content-length").unwrap_err().kind(), io::ErrorKind::InvalidData);
        let malformed = "POST / HTTP/1.1\r\nBroken Header\r\n\r\n";
        assert_eq!(single_header_value(malformed, "content-length").unwrap_err().kind(), io::ErrorKind::InvalidData);
        let transfer = "POST / HTTP/1.1\r\nTransfer-Encoding: chunked\r\n\r\n";
        assert_eq!(reject_transfer_encoding(transfer).unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert!(reject_invalid_header_name("Content-Length").is_ok());
        assert_eq!(reject_invalid_header_name("").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert_eq!(reject_invalid_header_name("Content Length").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert!(reject_non_crlf_header_lines(b"A: b\r\n\r\n").is_ok());
        assert_eq!(reject_non_crlf_header_lines(b"A: b\n\n").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert_eq!(reject_non_crlf_header_lines(b"A: b\rX").unwrap_err().kind(), io::ErrorKind::InvalidData);
    }

    #[test]
    fn framing_bounds_deadline_and_status_helpers_cover_failure_paths() {
        assert!(reject_full_request_buffer(1, 2).is_ok());
        assert_eq!(reject_full_request_buffer(2, 2).unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert!(reject_oversized_request(2, 2).is_ok());
        assert_eq!(reject_oversized_request(3, 2).unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert_eq!(declared_request_end(10, "2").unwrap(), 12);
        assert_eq!(declared_request_end(10, "").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert_eq!(declared_request_end(10, "2x").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert_eq!(declared_request_end(usize::MAX, "1").unwrap_err().kind(), io::ErrorKind::InvalidData);
        assert!(remaining_request_timeout(Instant::now() + Duration::from_secs(1)).is_ok());
        assert_eq!(remaining_request_timeout(Instant::now() - Duration::from_millis(1)).unwrap_err().kind(), io::ErrorKind::TimedOut);
        assert_eq!(normalize_read_error(io::Error::new(io::ErrorKind::WouldBlock, "block")).kind(), io::ErrorKind::TimedOut);
        assert_eq!(normalize_read_error(io::Error::other("boom")).kind(), io::ErrorKind::Other);
        ...
        assert_eq!(reason_phrase(415), "Unsupported Media Type");

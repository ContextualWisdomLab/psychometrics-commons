...
- Session start from a stored published release (`created_session_for_start` / `start_created_assessment_session` / `start_created_assessment_session_from_stored_release`, locking `instrument_release`) plus exact start replay after later persist Suspend or Retire and persist first-insert seal against missing, unpublished, or digest-mismatched reconstitution exists on Active PR #209; HTTP `POST /v1/sessions` remains target.

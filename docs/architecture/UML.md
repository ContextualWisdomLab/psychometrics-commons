...
- `ConsentSnapshot` records a purpose-specific decision and exact form/version evidence. Research consent is not inferred from service consent. Active reload work reconstructs the append-only `ConsentLedger` from durable `consent_event` rows after restart by physical insertion time so the latest purpose decision, including a same-millisecond revocation, is the one operators and later HTTP consent transport will see.

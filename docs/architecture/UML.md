...
- `src/item_delivery.rs`, `src/participant.rs`, `src/authorization.rs`, and `src/integration.rs` are protected-main domain evidence. `src/integration_delivery.rs` is also protected-main evidence through merged #264: it verifies the publisher receipt against the exact source/tenant/event identity before a fresh transaction persists the fenced attempt, but it still does not prove a live external publisher worker.
- Session start from a stored published release plus persist-backed `POST /v1/sessions` / `GET /v1/sessions/{session_ref}` is protected-main evidence through merged #232; command HTTP remains target.

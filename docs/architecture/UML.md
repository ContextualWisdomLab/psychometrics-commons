...
- `ConsentSnapshot` records a purpose-specific decision and exact form/version evidence. Research consent is not inferred from service consent. Active PR #140 reloads the append-only `ConsentLedger` from durable `consent_event` rows after restart so the latest purpose decision, including a same-millisecond revocation, is the one operators and later HTTP consent transport will see.

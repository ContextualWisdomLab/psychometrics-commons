...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. Active PR #202 `migrations/0022_participant_identity_link.sql` persists append-only link and link-end rows plus a derived current projection, treats unterminated history as the lookup/uniqueness source of truth, inspects projection drift after restore, rebuilds that projection before new account-link writes, and then frees an ended issuer-scoped subject for a later participant. `src/account_link_write.rs` refuses hosted dual-proof writes while that inspect reports drift and persists an authorized unlink from history; HTTP transport remains Target.

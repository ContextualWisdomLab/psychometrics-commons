...
- Physical `assessment_session` exists only on Active PR #61 (`migrations/0014_assessment_session.sql`): Created-only identity (participant, release, version, digest, locale, `created`, creation time) with no command history. Protected main still has the `src/session.rs` aggregate only.

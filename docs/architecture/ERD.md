...
- `assessment_session` creation identity is persisted only on the Active PR that adds `migrations/0023_assessment_session.sql`; protected main still has the `src/session.rs` domain aggregate without a physical session table.

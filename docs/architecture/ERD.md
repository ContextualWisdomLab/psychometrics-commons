...
- `response_event` is the accepted mid-session ledger. Physical `migrations/0020_response_event.sql` and `src/postgres_response_event.rs` persist/reload that prefix plus distinct observed/received clocks on Active PR #223; HTTP response transport remains Target.

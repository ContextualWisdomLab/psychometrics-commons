...
- Physical `assessment_session` exists only on Active PR #188 (`migrations/0014_assessment_session.sql`): Created identity (participant, release, version, digest, locale, creation time) plus a current-state projection. Physical `assessment_session_command` (`migrations/0016_assessment_session_command.sql`) stores append-only command history so later states reload by replaying Activate/Pause/Resume. A shorter persist than already stored fails closed and does not rewind that projection. Command persist locks the header row so concurrent shorter history cannot overwrite a later projection. Start requires a currently published release; load reconstitutes created identity without re-checking current publication eligibility. Protected main still has the `src/session.rs` aggregate only.

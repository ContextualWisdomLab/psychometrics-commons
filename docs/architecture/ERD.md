...
- `item_delivery_event` reflects the already-merged `src/item_delivery.rs` domain primitive. Physical `migrations/0004_item_delivery_evidence.sql` persists the tenant-bound ledger header and events; durable `instrument_version_ref` on `item_delivery_ledger` is **Active PR** evidence (`migrations/0032_item_delivery_instrument_version.sql`). HTTP delivery orchestration remains Target.

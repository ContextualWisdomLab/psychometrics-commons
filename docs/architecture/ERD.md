...
- Physical `assessment_session` exists only on Active PR #164 (`migrations/0014_assessment_session.sql`): Created-only identity (participant, release, version, digest, locale, `created`, creation time) with no command history. Load reconstitutes that identity without re-checking current publication eligibility. New sessions start through `start_created_assessment_session` or `start_created_assessment_session_from_stored_release`. Protected main still has the `src/session.rs` aggregate only.

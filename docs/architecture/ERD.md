...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. Active PR #147 `migrations/0022_participant_identity_link.sql` persists append-only link and link-end rows plus a derived current projection, treats unterminated history as the lookup/uniqueness source of truth, and reconciles the derived current projection on exact replay; HTTP transport remains Target.

...
- `response_event` is the in-progress answer ledger. Physical `migrations/0020_response_event.sql` on Active PR #208 stores opaque event identity, session binding, client idempotency, item version, payload digest, server sequence, and distinct observed/received timestamps; HTTP response transport remains Target. Completed `response_snapshot` persist is already on protected main.

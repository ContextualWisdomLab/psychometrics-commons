...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. The successor of Active PR #160 `migrations/0022_participant_identity_link.sql` persists append-only link and link-end rows plus a derived current projection, treats unterminated history as the lookup/uniqueness source of truth, reconciles the derived current projection on exact replay, and exposes hosted dual-proof write/recover commands that keep a recovered participant only when the current binding still matches the proof; HTTP transport remains Target.

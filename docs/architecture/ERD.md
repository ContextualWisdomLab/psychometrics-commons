...
- `consent_ledger` and `consent_event` persist the already-merged `src/consent.rs` append-only ledger. Physical `migrations/0005_consent_lifecycle.sql` and its write adapter are on protected main. Active PR #292 adds the nullable positive `event_sequence` ordering field, per-participant uniqueness, write fencing, and restart-safe reload; those additions are not protected-main truth until that exact PR head is integrated. HTTP consent transport and derived snapshot tables remain Target.
...
| unique `(participant_ref, event_sequence)` for non-null sequence | authoritative consent-decision order; `NULL` is reserved only for unsequenced legacy evidence during migration compatibility |

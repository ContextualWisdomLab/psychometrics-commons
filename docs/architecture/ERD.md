...
- `item_delivery_event` reflects the already-merged `src/item_delivery.rs` domain primitive. Physical `migrations/0004_item_delivery_evidence.sql` persists the tenant-bound ledger and events; Active reload reconstructs by `delivery_sequence` and fails closed on a gap or foreign tenant. HTTP item-delivery orchestration remains Target.

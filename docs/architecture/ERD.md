...
- `item_delivery_event` reflects the already-merged `src/item_delivery.rs` domain primitive. Physical `migrations/0004_item_delivery_evidence.sql` persists the tenant-bound ledger and events; reload reconstructs by stored `delivery_sequence` order and fails closed on a sequence gap or a foreign tenant (**Active PR** #224 restart-reload evidence). HTTP item-delivery orchestration remains Target.

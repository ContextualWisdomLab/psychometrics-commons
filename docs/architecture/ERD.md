...
- `response_snapshot` and `response_snapshot_entry` persist the already-merged completed prefix through `migrations/0010_response_snapshot.sql`. Active PR reload reconstructs that unique session snapshot after restart by `snapshot_sequence`; HTTP scoring transport remains Target.

...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. The Active PR successor of #183 persists append-only link and link-end rows plus a derived current projection, reconciles a stale projection so a later bind can reuse the freed subject, exposes dual-proof persist/recover commands, and serves `POST /v1/account-links` / `POST /v1/account-links/recover`; live Keyverse verification remains Target.

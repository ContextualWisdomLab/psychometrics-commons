...
- `result_snapshot` is the immutable personal-result identity already owned by `src/result.rs`. Physical `migrations/0007_result_snapshot.sql` persists that snapshot plus copied construct-level observations on Active PR #51; result-serving transport remains Target.

...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. Active PR #210 `migrations/0022_participant_identity_link.sql` persists append-only link and link-end rows plus a derived current projection, treats unterminated history as the lookup/uniqueness source of truth, inspects projection drift after restore, rebuilds that projection before new account-link writes, and then frees an ended issuer-scoped subject for a later participant. `src/account_link_write.rs` refuses hosted dual-proof writes while that inspect reports drift, persists an authorized unlink from history, and keeps recover only when the reconstructed current tenant/issuer/subject still match the proof. Active HTTP `src/account_link_http.rs` exposes persist, recover, and unlink on that write path; unlink recovers from the current proof and rejects a client `participant_ref`. Live Keyverse verification remains Target.

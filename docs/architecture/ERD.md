...
- `participant_identity_link` is the persistence target accepted by ADR-0020. The current `src/participant.rs` current-link fields are an application-domain first-link projection, not the future mutable persistence source of truth. The Active PR successor of #166/#160 persists append-only link and link-end rows plus a derived current projection, reconciles a stale projection so a later bind can reuse the freed subject, and exposes dual-proof persist/recover commands; HTTP transport remains Target.

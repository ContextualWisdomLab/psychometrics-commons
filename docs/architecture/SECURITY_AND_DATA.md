...
|---|---|---|
| Cross-tenant IDOR | valid user guesses another tenant's `result_ref` or persists another participant's consent ledger | server-side tenant/resource authorization; `persist_authorized_consent_ledger` refuses foreign participant/tenant writes; HTTP negative tests remain Target |

...
- Status: **IMPLEMENTED_ON_ACTIVE_PR** #427
- Protected-main baseline evaluated: `09534ef52c9307ce0dc559e9d908ebd715c641a1`
- Governing decisions: ADR-0005, ADR-0010, ADR-0015
- Product owner: Psychometrics Commons

This view maps the bounded response-event persistence slice carried by active PR #427. It is not protected-main truth until the unchanged reviewed/check-clean PR head is integrated. It does not redefine scoring, item delivery, consent, Gyeot collection, TEPP analysis, or any external service database.
...
`observed_at` is source-valid time for the accepted event. `received_at` is platform receipt time. The two values are intentionally distinct and immutable. `observed_at` must not be after `received_at`. Recovery evidence preserves both clocks; neither is synthesized from the other during reload.
...
## Evidence on active PR #427

The workstream carries real PostgreSQL contracts for persistence/replay, restart reconstruction, migration shape/reapplication, Rust-equivalent reference parity, sequence gaps, stored receipt identity conflicts, and exact-spelling rejection. Domain reconstruction tests prove a restarted prefix can continue toward the same scoring request. Exact-current-head Runtime CI, security/SAST, SBOM/provenance, independent non-author review, and repository-required workflows remain integration evidence rather than protected-main truth until they complete and the PR merges.

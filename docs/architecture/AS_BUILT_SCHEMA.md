...
## Active consent-ledger reload

`migrations/0005_consent_lifecycle.sql` and `src/postgres_consent.rs` already persist purpose-specific `consent_ledger` / `consent_event` rows on protected main. Active reload work adds `load_consent_ledger` and no new physical objects. After restart, a caller reconstructs the append-only ledger under `READ COMMITTED` by physical `created_at`. A `created_at` collision or an earlier `occurred_at` after a later insertion fails closed instead of being ordered by event identity. A missing participant is absent rather than an empty grant. Unknown stored labels fail closed. HTTP consent transport and outbox composition remain outside this slice.

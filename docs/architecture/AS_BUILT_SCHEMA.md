...
- Exact source identity is unique on `(enrollment_ref, source_system_ref, source_observation_ref)`. Exact replay is idempotent; source rebinding or stored-evidence mutation fails closed. UPDATE, DELETE, and TRUNCATE guards preserve the logical ERD's immutable-evidence semantics.

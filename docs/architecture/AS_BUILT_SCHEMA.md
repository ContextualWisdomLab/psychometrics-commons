...
The existing protected-main session HTTP GET must not be treated as an authorization boundary: an opaque `session_ref` is an identifier, not a capability. The active authorization implementation supplies a server-owned `ParticipantRecord` plus host-verified authenticated or current anonymous-session authority, then loads with an exact `(session_ref, participant_ref)` predicate. Unauthorized or foreign ownership is indistinguishable from a missing session. This does not duplicate `tenant_ref` into `assessment_session`; tenant ownership remains supplied by the server-owned participant record and the domain aggregate remains unchanged.

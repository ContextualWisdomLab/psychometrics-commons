...
| Machine-readable OpenAPI / AsyncAPI | **Active PR** session-command HTTP (`openapi/session-commands.yaml`) is as-built for `POST /v1/sessions/{session_ref}/commands` only; remaining families and AsyncAPI stay unimplemented | Do not list catalog, session create, response, result, consent, or event operations as deployed until those transports exist. |

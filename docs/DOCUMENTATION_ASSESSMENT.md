...
This reconciliation closes those architecture-definition gaps without promoting active PR work or target diagrams to shipped truth. PostgreSQL integration, scoring-job, scoring-request, result-snapshot, consent, inbox, data-rights, instrument-release, and health persistence are now protected-main implementation. Scoring-worker claim-next plus snapshot+job+outbox composition remains **Active PR** successor of #203, not protected-main truth. OpenAPI/AsyncAPI, physical DDL beyond actual migrations, deployed topology, measured SLO/RPO/RTO, certification, and instrument-release evidence remain implementation/evidence-gated and must not be fabricated.
...
| Traceability | **Repaired in this reconciliation** | Baseline now names exact protected-main `085ef4b…` in `docs/TRACEABILITY.md`, marks merged domain and persistence modules Implemented/Partial, and isolates scoring-worker claim-next composition as Active PR successor of #203. Must be updated after every material merge. |
...
Later protected-main merges added PostgreSQL adapters for integration evidence, scoring jobs and requests, result snapshots, consent, inbox consumption, data-rights, instrument releases, and operational-store readiness. `docs/TRACEABILITY.md` now evaluates baseline `085ef4b4714796a77fd4645eeb46b028f95929fc`. Scoring-worker claim-next plus result-snapshot composition on the Active PR successor of #203 remains `IMPLEMENTED_ON_ACTIVE_PR` until the exact reviewed/check-clean head is merged and protected main is refetched. Do not treat ready #203 or draft siblings #217, #214, or #207 as a second landing vehicle.

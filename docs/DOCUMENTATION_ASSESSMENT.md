...
| Machine-readable OpenAPI / AsyncAPI | **Active PR** #195 response-event HTTP (`openapi/responses.yaml`) is as-built for `POST /v1/sessions/{session_ref}/responses` only; remaining families and AsyncAPI stay unimplemented | Do not list catalog, session create, command, result, consent, or event operations as deployed until those transports exist. |

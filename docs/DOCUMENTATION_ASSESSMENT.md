...
| Machine-readable OpenAPI / AsyncAPI | **Partial / Active PR for item-delivery HTTP** | `openapi/item-deliveries.yaml` lists only the implemented `POST`/`GET` `/v1/sessions/{session_ref}/item-deliveries` operations, including resume fields, problem-type URIs, capability-URL authority, and next-action copy. Session, instrument, response, scoring, and result families must not be added until those transports exist. AsyncAPI remains not yet applicable. |

...
| Machine-readable OpenAPI / AsyncAPI | **Active PR** instrument catalog HTTP (`openapi/instruments.yaml`) is as-built for `GET /v1/instruments` and `GET /v1/instruments/{instrument_ref}` only; remaining families and AsyncAPI stay unimplemented | Do not list session, response, consent, or event operations as deployed until those transports exist. |

...
| Machine-readable OpenAPI / AsyncAPI | **Partial / session family only** | `openapi/sessions.yaml` is the as-built contract for `POST /v1/sessions` and `GET /v1/sessions/{session_ref}`. Do not publish remaining TRD families as deployed. AsyncAPI remains not applicable until durable event transport exists. |

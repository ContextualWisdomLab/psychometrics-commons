...
| Dead scoring worker is hidden by a healthy queue depth or enqueue-age bound | high | medium | implementation_in_progress | expired-lease count and oldest expired-lease timestamp against caller policy and one database-clock snapshot; HTTP probe wiring remains #132 |

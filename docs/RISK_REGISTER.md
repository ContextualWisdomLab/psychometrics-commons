...
| Purpose-bound audit evidence is mutated, rebound, or exposed across tenants | high | medium | implementation_in_progress | **Active PR #242** adds tenant-scoped append-only `audit_evidence_record`, exact replay/conflict rejection, mutation-blocking triggers, schema-shape checks, and concurrency/tenant-isolation tests; unchanged exact-head CI/security/review plus transport authorization and retention evidence remain required |

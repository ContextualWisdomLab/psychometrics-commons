...
A restricted linkage boundary maps an operational participant to a purpose/program-specific research pseudonym. Active PR #162 persistence for that mapping is not protected-main truth. A public-release fixture loads `public_research_identity` by program and never looks up a restricted linkage identity. The mapping:

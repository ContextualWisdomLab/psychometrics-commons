...
**Active PR** #203 scoring-worker result-snapshot composition (successor of #190, #200, #172, and #168) is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `ScoringRequest::from_persisted` and `load_scoring_request` reconstruct the version pin after restart. `plan_scoring_worker_result_attempt` validates the caller envelope before asking a request-bound engine, then binds one immutable `ResultSnapshot` to that pin or returns a typed retryable outage without an `event_ref`. `run_scoring_worker_attempt_with_result_snapshot` rejects a job/request mismatch before the engine runs, then persists the snapshot, fenced job, and stable outbox event in one caller-owned transaction, or schedules a retry with no snapshot and no terminal outbox. A missing or corrupt request, planner failure, or snapshot conflict leaves the leased job untouched. A later due claim after a retryable outage persists the snapshot; an exhausted retry budget quarantines without a snapshot or terminal outbox; a retry instant before the outage leaves the job leased. Prefer this ready head over draft #217, #214, #207, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those siblings and predecessors should not land separately. Live `fast-mlsirm` execution remains a later adapter behind the same engine trait. Modules added on this stack and not yet protected-main truth include `src/scoring_worker.rs`, `src/postgres_scoring_worker.rs`, and `migrations/0007_result_snapshot.sql` (result-snapshot write already exists on protected main; worker composition does not).

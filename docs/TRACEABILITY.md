...
**Active PR** #217 scoring-worker stored-request bind (successor of #207) is not protected-main truth until an unchanged reviewed/check-clean head is integrated. After a typed engine/transport outage the later due claim persists the real `result_snapshot` and one stable terminal `event_ref`. The job row must name the same `scoring_request_ref` the caller supplies; a mismatched pair fails closed before the engine runs. A permanent scientific failure writes a terminal cause with zero snapshots. Exhausted retry budget quarantines with zero outbox rows and no invented score. A retry instant earlier than the outage, a corrupt stored pin, a planner failure, or a conflicting pre-inserted snapshot keeps the job leased. Prefer this head over #214, #207, #203, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those predecessors should not land separately. Live `fast-mlsirm` execution and claim-next remain later adapters.

...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001`–`0003`, `0006`, `0011`, `0012` and matching adapters; **Active PR** #60 adds `migrations/0013_outbox_delivery_lease.sql` | remaining product aggregates, crash/restart restore acceptance |
...
├── postgres_integration.rs  # PostgreSQL integration evidence/delivery-attempt/lease persistence adapter
...
**Active PR** #60 exclusive outbox delivery-lease claim and expiry recovery on `integration_outbox` is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Live side-effect execution remains outside this slice.

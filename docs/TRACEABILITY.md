...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #178 persists append-only `participant_identity_link` history, restores the derived current projection on exact replay, inspects projection drift after restore, rebuilds that unique enforcer from unterminated history, and then frees an ended issuer-scoped subject for a later participant; this successor adds dual-proof write/recover commands that refuse new links while inspect reports drift; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** #178 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, inspects projection drift after restore, rebuilds the derived unique enforcer, and frees an ended issuer-scoped subject for a later participant; `src/account_link_write.rs` authorizes both proofs and refuses writes until that inspect is clean | HTTP unlink/relink transport and live Keyverse verification |
...
├── account_link_write.rs  # Active PR dual-proof persist/recover commands that refuse writes while inspect reports drift

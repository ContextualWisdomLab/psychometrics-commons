...
| Purpose-specific consent | PRD §5, §9.6 | TRD §12 | ADR-0006 | **Implemented** domain contract in `src/consent.rs` plus `migrations/0005_consent_lifecycle.sql` / `src/postgres_consent.rs` purpose-specific ledgers; HTTP transport remains Target. Active reload work reconstructs a persisted ledger after restart by physical `created_at` and fails closed on a `created_at` collision or non-monotonic insertion-order `occurred_at`, so a same-millisecond research revocation remains the latest decision |
...
├── postgres_consent.rs  # PostgreSQL purpose-specific consent ledger persist and restart reload
...
**Active PR** consent-ledger reload is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `load_consent_ledger` reconstructs one participant ledger from `consent_ledger` / `consent_event` under `READ COMMITTED` after `FOR SHARE` on the header. Reconstruct uses physical `created_at` insertion order. A `created_at` collision or an earlier `occurred_at` after a later insertion fails closed instead of being ordered by event identity. This does not add outbox composition (see #142) or HTTP consent transport.

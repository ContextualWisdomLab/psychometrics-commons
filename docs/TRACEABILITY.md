...
| Headless replaceable clients | PRD §7 | TRD §1, §18; C4 | ADR-0001, ADR-0002 | Architecture established; Active PR #195 response-event HTTP records answers on an Active session; remaining public families are Target |
...
| Only Active accepts responses | TRD §5–6 | `SessionState::accepts_responses` + response tests; **Active PR** #195 `POST /v1/sessions/{session_ref}/responses` rejects Created/Paused | durable transport on protected main |

...
| Operation-scoped capability health | PRD §7, §13 | `docs/OPERABILITY.md` §3–4; Deployment/Operations | ADR-0011, ADR-0017 | **Implemented** domain health/readiness contract in `src/health.rs` plus `src/postgres_health.rs` PostgreSQL major/write-readiness and caller-declared relation presence; **Active PR** #82 observes aggregate outbox, inbox-consumption, and data-rights backlog counts/ages and applies partial readiness indexes through `apply_backlog_health_index_migration`; **Active PR** #113 observes queued/leased/retry-scheduled/quarantined scoring-job counts and created-at age through `apply_scoring_job_migration`; this successor also counts leased rows whose `active_lease_expires_at_unix_ms` is already past one database-clock snapshot and returns that oldest expired-lease timestamp so a dead worker cannot hide behind a healthy enqueue-age bound or a live future expiry; HTTP probes remain #132; measured thresholds and deployment-profile evidence remain Target |
...
| Unknown/stalled backlog or unknown/incompatible integrity blocks new state-changing work | Operability §3, §6, §8 | **Implemented** domain contract in `src/health.rs`; `src/postgres_health.rs` fails closed on unsupported/read-only PostgreSQL or a missing required relation; **Active PR** #82 classifies durable integration and data-rights backlog evidence against an explicit caller policy; **Active PR** #113 classifies queued/leased/retry-scheduled/quarantined scoring-job evidence the same way; this successor also classifies expired-lease count and exposes the oldest already-expired lease timestamp | HTTP probe wiring on #132, measured deployment-profile thresholds, alerting, and failure-injection evidence |
...
├── postgres_health.rs  # PostgreSQL major/write-readiness, relation-integrity, and Active PR operational-backlog probes including scoring-job queues
...
**Active PR** #82 PostgreSQL operational-backlog observation is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Aggregate outbox, inbox-consumption, and data-rights counts/ages are classified only against an explicit caller policy. Partial readiness indexes ship in `migrations/0020_backlog_health_indexes.sql` and must be applied through `apply_backlog_health_index_migration` after the owning table migrations; a private test `include_str!` is not a product apply path. **Active PR** #113 adds the same content-free observation for queued, leased, retry-scheduled, and quarantined scoring work, with readiness indexes applied through `apply_scoring_job_migration`. This successor also observes expired leased rows against one database-clock snapshot, returns the oldest already-expired lease timestamp, and applies `migrations/0022_scoring_job_expired_lease_health_indexes.sql` through the same product apply path. Prefer this head over merging #113 or #131 alone when dead-worker visibility is in the same integration window. Draft #114 must renumber its identity-link migration past `0022` before it leaves draft. HTTP probes remain #132. Deployment-profile thresholds remain outside this slice.

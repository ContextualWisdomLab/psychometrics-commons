...
| Idempotent response events | PRD §9.2 | TRD §6 | ADR-0005, ADR-0010 | **Implemented** in `src/response.rs` with canonical SHA-256 payload-digest identity; **Active PR** #53 carries `migrations/0009_response_event.sql`, the `postgres_response` adapter, and real PostgreSQL tests for exact event replay plus fail-closed digest/item/client-identity/sequence rebinding. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP response transport remains outside this slice.

...
| Only Published release accepts new sessions | TRD §7 | `PublicationState::accepts_new_sessions` in `src/instrument.rs`; `AssessmentSession` creation copies exact published release/version/locale provenance and fails closed on unpublished eligibility or locale mismatch | **Active PR** #121 start composition (`created_session_for_start`) plus persist/load without re-checking current eligibility; HTTP session-creation remains missing |

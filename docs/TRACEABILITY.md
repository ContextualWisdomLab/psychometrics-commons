...
| Idempotent response events | PRD §9.2 | TRD §6 | ADR-0005, ADR-0010 | **Implemented** in `src/response.rs` with canonical SHA-256 payload-digest identity; **Active PR** #208 persists and reloads `response_event` under `READ COMMITTED`; HTTP response transport remains Target |
...
| Conflicting idempotency replay fails closed | TRD §6 | `src/response.rs`; **Active PR** #208 `migrations/0020_response_event.sql` unique `(session_ref, client_event_ref)` and `(session_ref, server_sequence)` | HTTP transport rejection test |
...
**Active PR** #208 response-event persist/load is not protected-main truth until an unchanged reviewed/check-clean head is integrated. In-progress answers persist in `response_event` with observed/received time, exact replay, and fail-closed rebinding so a two-item path survives restart. Recovery COPY restores those rows and asserts first-write observed/received instants. AS_BUILT names the physical slice. Reload exposes stored times as a persist-side projection, rejects pre-epoch stored clocks, and continues the scoring prefix after item 3. Completed snapshot persist remains the existing protected-main adapter. HTTP response transport remains outside this slice. Protected-main `#76` data-rights processing-start persistence is already integrated and is not restated here as active work. Prefer this head over #201, #174, #182, and #53.

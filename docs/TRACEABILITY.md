...
| Purpose-specific consent | PRD §5, §9.6 | TRD §12 | ADR-0006 | **Implemented** domain contract in `src/consent.rs`; grant/revoke/inspect are bound to participant ownership; **Active PR #49** adds PostgreSQL ledger/event persistence; public transport remains Target |
...
**Active PR** #49 (`feat: persist PostgreSQL purpose-specific consent ledgers`) carries `migrations/0005_consent_lifecycle.sql`, the `postgres_consent` adapter, and real PostgreSQL tests for exact ledger/event replay, research-revocation append, and fail-closed conflicting replay. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP consent transport remains outside this slice.

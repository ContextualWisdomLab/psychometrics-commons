...
**Active PR** #196 additionally locks `instrument_release` with `SELECT … FOR UPDATE` during persist classification so a Duplicate published result cannot lose the row to a concurrent Suspend or Retire before the caller transaction ends. That persist-side lock is not protected-main truth. This successor head also locks `load_published_instrument_release` with `SELECT … FOR UPDATE` so `start_created_assessment_session_from_stored_release` holds the publication row until commit. Prefer this head over #196 for combined Created-only publication locks. Prefer #180 for command-history + start lock. HTTP session-creation remains #149. #164 remains the unlocked stored-release start predecessor.

...
**Active PR** #91 PostgreSQL-backed operator health HTTP is not protected-main truth until an unchanged reviewed/check-clean head is integrated. GET `/live` answers process liveness without store I/O. GET `/ready` observes a live operational snapshot after accept; a bare `/ready` requires `postgres_operational_store`. Driver errors are not exposed. Public/admin product routes, measured backlog thresholds, TLS, and deployment-profile evidence remain outside this slice. Persist-backed session HTTP, exclusive outbox delivery-lease persistence, claim-next scoring-job poll, and longitudinal observation-time ingest are already on the current protected-main head.

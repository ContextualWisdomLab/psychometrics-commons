...
|---|---|---|---|
| Server-authoritative session state | TRD §5 | `src/session.rs` + session contract tests, including published-release/locale binding at creation | **Active PR** #153 persist/load created-session identity and later command history with exact/conflicting replay, fail-closed stale shorter history, header-row `FOR UPDATE` before command insert/count, and published-release start boundary; HTTP/API concurrency remains missing |
...
| Only Published release accepts new sessions | TRD §7 | `PublicationState::accepts_new_sessions` in `src/instrument.rs`; `AssessmentSession` creation copies exact published release/version/locale provenance and fails closed on unpublished eligibility or locale mismatch | **Active PR** #153 start boundary (`created_session_for_start` / `start_created_assessment_session`) plus persist/load without re-checking current eligibility; HTTP session-creation remains missing |

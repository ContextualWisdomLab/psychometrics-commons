...
| Research identity separation | PRD §5, §11 | TRD §14; ERD restricted linkage | ADR-0003, ADR-0006, ADR-0007, ADR-0020 | **Partially implemented**: research-contribution identity separation exists on evaluated main; **Active PR** restricted linkage persistence binds one operational participant to a program-scoped research identity, keeps multiple programs from collapsing into one pseudonym, and projects only research identities into a public release view |
...
├── postgres_research_identity_linkage.rs  # Active PR restricted operational-to-research identity persistence (not protected-main truth)
...
├── research_identity_linkage.rs  # Active PR restricted linkage domain and public-release projection (not protected-main truth)
...
└── 0025_research_identity_linkage.sql  # Active PR restricted linkage persistence (not protected-main truth)
```

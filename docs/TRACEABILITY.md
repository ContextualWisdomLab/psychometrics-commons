...
**Active PR** #82 PostgreSQL operational-backlog observation is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Aggregate outbox, inbox-consumption, and data-rights counts/ages are classified only against an explicit caller policy. Partial readiness indexes ship in `migrations/0020_backlog_health_indexes.sql` and must be applied through `apply_backlog_health_index_migration` after the owning table migrations; a private test `include_str!` is not a product apply path. **Active PR** #113 adds the same content-free observation for queued, leased, retry-scheduled, and quarantined scoring work, with readiness indexes applied through `apply_scoring_job_migration`. This successor also observes expired leased rows against the database clock and applies `migrations/0022_scoring_job_expired_lease_health_indexes.sql` through the same product apply path. Prefer this head over merging #113 alone when dead-worker visibility is in the same integration window. Draft #114 must renumber its identity-link migration past `0022` before it leaves draft. HTTP probes remain #111. Deployment-profile thresholds remain outside this slice.

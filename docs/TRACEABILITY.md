...
| Immutable result provenance | PRD §3.1, §9.4 | TRD §9 | ADR-0004, ADR-0010 | **Implemented** in `src/result.rs` plus `migrations/0007_result_snapshot.sql` / `src/postgres_result_snapshot.rs` persist; reload-after-restart is Active PR work, and result-serving transport remains Target |

...
**Active PR** for scoring-worker result-snapshot composition remains #203 until an unchanged reviewed/check-clean head is integrated. This branch adds `claim_next_scoring_job`: a worker that does not already know the job identity claims the oldest due queued or retry-scheduled row with `FOR UPDATE SKIP LOCKED`, receives the bound `scoring_request_ref` and fencing lease, and leaves an empty due set as `None`. Prefer #203 over #200, #190, #172, #168, #167, #155, #136, #115, and #69 for the snapshot/retry composition; do not land those predecessors separately. Live `fast-mlsirm` execution and scoring HTTP remain later adapters.

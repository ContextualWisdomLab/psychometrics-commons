...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** successor of #202 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, inspects projection drift after restore, rebuilds the derived unique enforcer, and frees an ended issuer-scoped subject for a later participant; `src/account_link_write.rs` authorizes both proofs, refuses new links until that inspect is clean, persists an authorized unlink so recovery returns `None` until relink, and rejects a recovered participant whose current binding no longer matches the proof | HTTP unlink/relink transport and live Keyverse verification |
...
**Active PR** successor of #202 participant identity-link persistence is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Prefer this recover-binding head over #202, #192, #183, #178, #176, #173, #169, #160, #158, #147, #133, #124, and #114. This head keeps the inspect-then-reconcile persist line and adds `src/account_link_write.rs` so a hosted adapter authorizes both current proofs, refuses new account-link writes while inspect reports drift, persists an authorized unlink from unterminated history, recovers the same `participant_ref` after restart, and keeps that record only when the reconstructed current tenant/issuer/subject still match the proof. `migrations/0022_participant_identity_link.sql` and `src/postgres_participant_identity_link.rs` persist `assessment_participant`, append-only `participant_identity_link` / `participant_identity_link_end` evidence, and a derived `current_participant_identity_link` projection. Persist applies each link and then its matching ends so a complete unlink+relink aggregate can be written in one transaction. Exact replay then restores or clears that derived projection. After restore, `inspect_identity_link_current_projection_drift` reports missing or stale unique-enforcer rows without mutating them, and `reconcile_identity_link_current_projections` rebuilds every current row from unterminated history. Both fail-close on two unterminated holders of the same issuer-scoped subject or two unterminated links on one participant. After that inspect-then-reconcile path, an ended issuer-scoped subject can bind to a later participant. Unterminated issuer-scoped subjects remain the lookup and uniqueness source of truth. HTTP account-link transport and live Keyverse token verification remain outside this slice. Migration `0021` remains reserved for #113 scoring-job health indexes.

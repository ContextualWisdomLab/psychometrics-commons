...
| Immutable result provenance | PRD §3.1, §9.4 | TRD §9 | ADR-0004, ADR-0010 | **Implemented** in `src/result.rs`; authorized `GET /v1/results/{result_ref}` transport is protected-main truth through merged #257, while active PR #420 adds the metadata-only `HEAD` companion and RFC 9110 `Allow: GET, HEAD` contract without changing score authority |
| Authorized immutable personal result read | PRD §3.1, §9.4 | TRD §18 `GET` / `HEAD /v1/results/{result_ref}`; result authorization boundary | ADR-0003, ADR-0010 | **Implemented on protected main** for authorized `GET` through merged #257; **Active PR #420** adds `HEAD` with identical authorization/routing metadata and an empty response body, plus `Allow: GET, HEAD` on method rejection. The #420 behavior is not protected-main truth until merge |
...
| Immutable result provenance | PRD §3.1, §9.4 | TRD §9 | ADR-0004, ADR-0010 | **Implemented** in `src/result.rs`; authorized result serving via `GET` is protected-main truth through merged #257, while active #420 extends only the HTTP metadata surface with `HEAD` / `Allow` |
...
| Authorized result read does not expose cross-tenant existence | TRD §11, §18; Security/Data | Protected-main `src/result_http.rs` from merged #257 authorizes server-owned participant/result records before comparing the requested reference and uses safe RFC 9457-style problem responses for `GET`; active #420 applies the same authorization/routing path to metadata-only `HEAD` and advertises `Allow: GET, HEAD` only for method rejection | PostgreSQL resource loading and cross-tenant HTTP E2E remain before GA; #420 HEAD/Allow evidence remains active-PR-only until integrated |
...
Merged #257 authorized immutable personal result reads are protected-main truth: `src/result_http.rs` serves exact `GET /v1/results/{result_ref}` only after checking server-owned participant/result authorization before route identity, and it serializes the stored immutable provenance without recomputation. **Active PR #420** extends this same result-read family with metadata-only `HEAD` and RFC 9110 `Allow: GET, HEAD`; those #420 semantics remain active-PR-only until an unchanged reviewed/check-clean head is integrated. PostgreSQL result loading and cross-tenant HTTP E2E remain before GA.

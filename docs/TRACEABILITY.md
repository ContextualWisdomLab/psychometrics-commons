...
├── health_http.rs    # Active PR #132 (successor to #122) operator GET /live and GET /ready probes plus a bounded-timeout TCP listener that keeps serving after a dropped probe (not protected-main truth)
├── health_process.rs # Active PR #132 (successor to #122) binds HEALTH_LISTEN_ADDR or PORT and observes DATABASE_URL only for /ready (not protected-main truth)
...
├── postgres_health.rs  # PostgreSQL major/write-readiness, relation-integrity, and Active PR #132 (successor to #122) operational snapshot
├── postgres_health_http.rs  # Active PR #132 (successor to #122) observes PostgreSQL only for GET /ready; /live stays store-I/O free; serve loop keeps accepting after a dropped probe (not protected-main truth)

...
| Sequence-aware item delivery evidence | PRD §3.1, §9 | TRD §5–7 | ADR-0005, ADR-0010, ADR-0015 | **Implemented** domain primitive in `src/item_delivery.rs` on the evaluated baseline. Current protected `main` at `503a4e640eeba0f5e126fa4c4078d8d21aebb93b` additionally contains the PostgreSQL 18 `item_delivery_ledger` / `item_delivery_event` persistence contract in `migrations/0004_item_delivery_evidence.sql` and `src/postgres_item_delivery.rs`; public/API delivery orchestration remains Target. Active PR #276 hardens that shipped persistence boundary to Rust-equivalent Unicode reference validation and migration-time revalidation; it is not protected-main truth until merged. |
...
| Deterministic narrative fallback | PRD §3.2, §9.5 | TRD §17; Architecture narrative view | ADR-0009, ADR-0010, ADR-0018 | **merged #287** hardens the narrative slice so published narrative/rule references must already use canonical opaque spelling; this protection is protected-main truth |
...
| Item delivery sequence is positive and evidence-safe | TRD §5–7; ADR-0015 | `src/item_delivery.rs` + item-delivery domain tests on the evaluated baseline; current protected `main` additionally persists ledger/event evidence through `migrations/0004_item_delivery_evidence.sql` and `src/postgres_item_delivery.rs` | public/API delivery orchestration and broader end-to-end acceptance; Active PR #276 reference-parity hardening must merge before that stricter database guarantee is protected-main truth |
...
**Current protected-main delta:** after the historical module-map baseline above, protected `main` added `migrations/0004_item_delivery_evidence.sql` and `src/postgres_item_delivery.rs` for the durable item-delivery ledger/event subset. ADR-0005 and ADR-0010 continue to govern the domain/provenance semantics; ADR-0015 governs its PostgreSQL persistence and transaction boundary. Active PR #276 changes only the reference-integrity and reapplication evidence of that shipped persistence slice. ERD ownership/cardinality, public API families, cross-service dependency direction, scoring authority, and confidential-response boundaries are unchanged by #276, so those artifacts do not acquire a new implementation claim from this hardening PR.
...
Merged #257 authorized immutable personal result reads are protected-main transport evidence through `src/result_http.rs`: exact `GET /v1/results/{result_ref}` targets only, server-owned participant/result authorization before identity comparison, no result existence oracle for unauthorized actors.

...
| Operation-scoped capability health | PRD §7, §13 | `docs/OPERABILITY.md` §3–4; Deployment/Operations | ADR-0011, ADR-0017 | **Implemented** domain health/readiness contract in `src/health.rs` plus `src/postgres_health.rs` PostgreSQL major/write-readiness and caller-declared relation presence; **Active PR** #111 (successor to #91) binds GET `/live` and GET `/ready` to a TCP listener that can serve until accept fails, answers `/live` without store I/O, and observes PostgreSQL only for `/ready` (bare `/ready` requires `postgres_operational_store`) without exposing driver errors; measured thresholds and deployment-profile evidence remain Target |
...
| Liveness is distinct from operation readiness | Operability §3–4; ADR-0017 | **Implemented** in `src/health.rs` and `src/postgres_health.rs`; **Active PR** #111 (successor to #91) exposes GET `/live` independently from GET `/ready` on a bound TCP listener that can serve until accept fails and does not observe PostgreSQL for `/live` | metrics and deployment-profile acceptance |
| Optional capability outage does not fail unrelated work | Operability §3–4; ADR-0011/0017 | **Implemented** in `src/health.rs` and `src/postgres_health.rs`; **Active PR** #111 keeps `/ready?capability=` fail-closed for unknown or unsafe required capabilities | degraded-mode transport/integration tests |
...
├── health_http.rs    # Active PR #111 operator GET /live and GET /ready probes plus a bounded-timeout TCP listener that can serve until accept fails (not protected-main truth)
...
├── postgres_health.rs  # PostgreSQL major/write-readiness, relation-integrity, and Active PR #111 operational snapshot
├── postgres_health_http.rs  # Active PR #111 observes PostgreSQL only for GET /ready; /live stays store-I/O free; serve loop retries until accept fails (not protected-main truth)
...
**Active PR** #111 (successor to #91) PostgreSQL-backed operator health HTTP is not protected-main truth until an unchanged reviewed/check-clean head is integrated. GET `/live` answers process liveness without store I/O. GET `/ready` observes a live operational snapshot after accept; a bare `/ready` requires `postgres_operational_store`. `serve_health_http` and `serve_postgres_health_http` keep accepting until accept fails so a load balancer can keep probing. Driver errors are not exposed. Public/admin product routes, measured backlog thresholds, TLS, and deployment-profile evidence remain outside this slice. Persist-backed session HTTP, exclusive outbox delivery-lease persistence, claim-next scoring-job poll, and longitudinal observation-time ingest are already on the current protected-main head.

...
- Date: 2026-08-12
- Evaluated protected-main implementation baseline: `090d88094b38f102d7d2b9e8f94ebf773bd3dc3c`
...
| Operation-scoped capability health | PRD §7, §13 | `docs/OPERABILITY.md` §3–4; Deployment/Operations | ADR-0011, ADR-0017 | **Implemented** domain health/readiness contract in `src/health.rs`; HTTP probes, live dependency checks, measured thresholds, metrics/alerts, and deployment evidence remain Target |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | architecture target only; evidence adapter is **Active PR #24** | merge exact-head persistence work + migrations + concurrency/crash/restore acceptance |
...
| Liveness is distinct from operation readiness | Operability §3–4; ADR-0017 | `src/health.rs` models liveness separately from operation-scoped readiness and independent capabilities | live transport probes, dependency observations, metrics, and deployment-profile acceptance |
| Optional capability outage does not fail unrelated work | Operability §3–4; ADR-0011/0017 | `src/health.rs` evaluates only capabilities required by the selected operation and fails unknown required capability closed | real adapter classification and degraded-mode integration tests |
| Unknown/stalled backlog or unknown/incompatible integrity blocks new state-changing work | Operability §3, §6, §8 | `src/health.rs` fail-closed domain contract | persistence/job backlog metrics, migration/schema probes, alerting, and failure-injection evidence |
...
Current protected-main Rust module surface on `090d88094b38f102d7d2b9e8f94ebf773bd3dc3c`:
...
├── health.rs         # operation-scoped liveness/readiness/capability evidence

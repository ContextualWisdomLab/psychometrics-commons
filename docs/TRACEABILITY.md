...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** #176 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`; `src/account_link_write.rs` authorizes both proofs before persist, recovers from a still-valid account proof, and drops a loaded participant whose current binding no longer matches that proof | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |

...
| Cross-cutting tenant/task authorization | PRD §7, §9 | TRD §11; Security/Data | ADR-0001, ADR-0003 | **Implemented** fail-closed domain gate in `src/authorization.rs` binds consent operations to participant-owned `ConsentLedger` / `ManageOwnConsent`. Active PR wires that gate through `src/postgres_consent_authorization.rs` before durable consent writes; public-transport integration remains Target |
| Purpose-specific consent | PRD §5, §9.6 | TRD §12 | ADR-0006 | **Implemented** domain contract in `src/consent.rs` plus `migrations/0005_consent_lifecycle.sql` / `src/postgres_consent.rs` purpose-specific ledgers; HTTP transport remains Target. Prefer consent/outbox landing head #134 over #123/#120/#112/#70 |
...
| Sensitive authorization is tenant- and task-bound | TRD §11; Security/Data | `src/authorization.rs` fail-closed authorization context/gates bind consent operations to participant ownership; `src/postgres_consent_authorization.rs` requires `ManageOwnConsent` before a consent write is authorized | route/repository integration + HTTP cross-tenant E2E tests |
...
├── postgres_consent_authorization.rs  # Active PR: ManageOwnConsent before durable consent writes
...
**Active PR** consent authorization write-path gate is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `authorize_consent_propagation` requires the authenticated tenant participant to hold `ManageOwnConsent` on their own ledger before any later durable consent or outbox composition. A foreign participant, foreign tenant, missing participant identity, or numeric tenant fails closed and writes nothing. Service-operation consent still does not create research contribution. Prefer consent/outbox landing head #134 (`729a2bd`) over #123/`6643041`, #120/`3f72446`, #112/`040bcf7`, and #70/`3180620`. HTTP `POST /v1/consents` remains Target.

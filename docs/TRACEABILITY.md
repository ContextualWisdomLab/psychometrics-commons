...
**Active PR** #213 instrument-release startable-catalog listing is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Prefer this head over #193, #179, and #171 for catalog-from-store. After restart, `list_startable_instrument_releases` returns stored Published forms ordered by `instrument_ref`, exact BCP 47 `locale`, then `release_ref`. Draft, Review, Suspended, and Retired rows are omitted, including a row that was Published and later Suspended on the same `release_ref`. A corrupt Published row fails closed as a stored-snapshot reconstruction error so the catalog cannot treat damaged evidence as startable. `load_instrument_release` still reconstructs any stored publication state by `release_ref` and is not the session-start gate. Exact persist replay of a listed snapshot stays Duplicate. Publication event history and bound evidence are not stored by this adapter. This is complementary to #205 `load_published_instrument_release` / first-insert seal, which is the session-start landing vehicle (prefer over #180), and to #165, which is in-process HTTP catalog only. Do not merge this slice in parallel with #205; rebase after #205. Do not fold HTTP or session start into this adapter list. Do not merge #164 or #179 in parallel. `src/postgres_instrument_release.rs` and `migrations/0006_instrument_release.sql` are the adapter/migration for this slice.

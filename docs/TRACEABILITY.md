...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #133 persists append-only `participant_identity_link` history, reloads it after restart, recovers the participant from unterminated history, restores a missing current projection on exact replay, and rejects a second unterminated subject in the database; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** #133 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, restores a missing current projection on exact replay, and rejects a second unterminated issuer-scoped subject in the database | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |

...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #206 adds hosted dual-proof write/recover/unlink commands in `src/account_link_write.rs`, keeps a recovered participant only when the current tenant/issuer/subject still match the proof, and ends that binding from a still-valid current proof; **Active PR** #222 binds any later account-linked capability to the current `link_event_ref` so unlink or rebound invalidates a previously issued grant; **Active PR** #158 remains the store-wide restore-reconcile vehicle. Both include #147 persist.; HTTP/Keyverse token verification remain Target |
...
**Active PR** #206 hosted dual-proof write/recover/unlink commands and **Active PR** #158 store-wide restore reconcile are not protected-main truth until an unchanged reviewed/check-clean head is integrated. **Active PR** #222 adds `grant_account_linked_capability` / `accept_account_linked_capability` so a recovered `participant_ref` cannot keep acting as the Keyverse account after unlink or rebound; the grant is bound to the current `link_event_ref`. Prefer #206 for persist unlink and #222 for the post-recover account-capability gate. Prefer those heads over #176, #160, #147, #133, #124, or #114. Keep #158 for restore reconcile. Keep #202 as the inspect-line unlink vehicle; do not merge it as a substitute for #176's recover binding check. Do not merge #176, #160, #147, #133, #124, or #114. Merge #158 and #206 sequentially after rebase; they share persist files. `migrations/0022_participant_identity_link.sql` and `src/postgres_participant_identity_link.rs` persist `assessment_participant`, append-only `participant_identity_link` / `participant_identity_link_end` evidence, and a derived `current_participant_identity_link` projection. `src/account_link_write.rs` authorizes both current proofs, persists that history, recovers the same `participant_ref` from a still-valid authenticated account proof, keeps that participant only when the current tenant/issuer/subject still match the proof, ends a matching current binding from a still-valid account proof so recover then returns `None`, and re-checks that current `link_event_ref` before treating the recovered participant as an account-linked capability. Persist applies each link and then its matching ends so a complete unlink+relink aggregate can be written in one transaction. Exact replay then restores or clears that derived projection. Unterminated issuer-scoped subjects are the lookup and uniqueness source of truth, so a returning account still recovers the same `participant_ref` when the derived projection is missing. HTTP account-link transport and live Keyverse token verification remain outside this slice. Migration `0021` remains reserved for #113 scoring-job health indexes.

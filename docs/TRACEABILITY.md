...
**Active PR** claim-next scoring-worker orchestration (successor of #217) is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Claim-next leases the oldest due queued or retry-scheduled job with `FOR UPDATE SKIP LOCKED`, reads `scoring_request_ref` from that job row, and calls only `run_scoring_worker_attempt_with_result_snapshot`. A later due claim after a typed engine outage persists the real `result_snapshot` and one stable terminal `event_ref` without a caller-supplied request pin. An empty or not-yet-due queue returns a typed no-due-job error and invents no score. Prefer this head over #217, #214, #207, #203, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those predecessors should not land separately. Live `fast-mlsirm` execution and scoring HTTP remain later adapters.

...
**Active PR** #302 adds durable PostgreSQL proof-digest/binding persistence and restart reload; it is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Raw anonymous bearer proof is not persisted; only the canonical proof digest and exact product-owned tenant/participant/session binding are stored by this active-PR implementation. Raw bearer issuance/hashing, HTTP issuance/audience/max-TTL enforcement, and the complete public journey remain Target.
...
Merged #257 authorized immutable personal result reads are protected-main transport evidence through `src/result_http.rs`: it accepts only exact `GET /v1/results/{result_ref}` targets, checks server-owned participant/result authorization before identity comparison, returns no result existence oracle to an unauthorized actor, and serializes the stored immutable provenance without recomputation.

...
| Immutable result provenance | PRD §3.1, §9.4 | TRD §9 | ADR-0004, ADR-0010 | **Implemented** in `src/result.rs`; **Active PR** #51 persists copied provenance and observations; result-serving transport is Target |
...
| Historical result does not mutate | TRD §9 | `src/result.rs` snapshot semantics; **Active PR** #51 inserts immutable rows and never updates them | API supersession tests |

...
| Only Published release accepts new sessions | TRD §7 | `PublicationState::accepts_new_sessions` in `src/instrument.rs`; protected-main `start_created_assessment_session_from_stored_release` locks publication evidence and persists HTTP create/reload; load still restores created identity without re-checking current eligibility | **Active PR** family-scoped catalog discovery (#303/#304) adds advisory persisted Published candidate discovery with non-locking validation while final session start still reloads/locks the exact release; command HTTP and the rest of the assessment transport remain missing |

...
| Personal JSON and human-readable result export | PRD §3.1, §9.4 | TRD §18 `POST /v1/results/{result_ref}/exports`; ADR-0010 export provenance | ADR-0010 | **Implemented** domain copy through merged #231, delivery guard through merged #249 (`src/result_export_authorization.rs`), and authorized HTTP transport through merged #256. **Active PR #421** changes presentation only: machine-readable JSON and typed fields keep exact owner/version/digest/time provenance while localized and shared human-readable participant copy omit internal identifiers and explain the retained evidence in ordinary language. |
...
| Personal JSON and human-readable result export | PRD §3.1, §9.4 | TRD §18 `POST /v1/results/{result_ref}/exports`; ADR-0010 export provenance | ADR-0010 | **Implemented** domain copy through merged #231, delivery guard through merged #249 (`src/result_export_authorization.rs`), and authorized HTTP transport through merged #256 (`src/result_export_http.rs`, `openapi/result-exports.yaml`). **Active PR #421** retains server-authoritative export/render time as typed provenance and removes internal identity/version metadata only from human-readable copy. |
...
| Korean/English exact locale versions | PRD §3.1, §9.9 | TRD §28; instrument release + locale governance | ADR-0013, ADR-0019 | **Partially implemented**: locale is pinned/validated by `src/instrument.rs`; merged #259 ships protected-main exact `ko-KR`/`en-US` participant report labels that copy immutable scores. **Active PR #421** keeps exact locale fail-closed while replacing internal provenance jargon/identifiers in participant copy with reviewed plain-language evidence explanations; real form content, rights, translation, invariance, HTTP delivery, and accessible reference-client serving remain Target |
...
| EMA/ESM longitudinal flow | PRD §4 | TRD §16; UML longitudinal sequence; logical ERD extension | ADR-0008 | External Gyeot/TEPP dependencies + Target Commons enrollment/orchestration adapter; `src/longitudinal_observation.rs` records validity, recorded, received, and ingested clocks with explicit membership shares. **Implemented** on the evaluated protected-main baseline through merged #248: PostgreSQL 18 persistence for immutable normalized observation records and membership shares via `migrations/0031_longitudinal_observation.sql` and `src/postgres_longitudinal_observation.rs`; enrollment persistence, HTTP, Gyeot collection, and TEPP kernels remain Target |
...
| Authorized result read does not expose cross-tenant existence | TRD §11, §18; Security/Data | Active PR #257 `src/result_http.rs` authorizes server-owned participant/result records before comparing the requested reference and uses safe RFC 9457-style problem responses | protected-main transport, PostgreSQL resource loading, and cross-tenant HTTP E2E |
| Result export includes machine-readable provenance and the same scores | ADR-0010; PRD §3.1 | Protected-main `src/result_export.rs` copies snapshot scores, standard errors, dispositions, owner identity, and version provenance into JSON and its original human-readable report; authorized HTTP transport shipped through merged #256. **Active PR #421** keeps those values and the server-authoritative export time in typed/machine-readable evidence while removing internal identifiers only from human-readable copy. | contract tests on `tests/result_export_http.rs`; Active PR #421 participant-copy regression tests |
| Result export includes machine-readable provenance and the same scores | ADR-0010; PRD §3.1 | Protected-main `src/result_export.rs` copies immutable score/provenance evidence; **Active PR #421** makes the human-readable view privacy-minimized without changing numeric or machine-readable provenance and retains typed export/render timestamps | authorized HTTP transport shipped through merged #256; Active PR #421 exact-head checks/review |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001_integration_delivery.sql`, `migrations/0002_scoring_job_state.sql`, `migrations/0003_data_rights_propagation.sql`, `migrations/0005_consent_lifecycle.sql`, `migrations/0006_instrument_release.sql`, `migrations/0011_scoring_request.sql`, `migrations/0012_integration_consumption.sql`, matching adapters, protected-main longitudinal observation persistence (`migrations/0031_longitudinal_observation.sql`), and PostgreSQL operational-store readiness | remaining product aggregates, crash/restart restore acceptance |
...
├── localized_result_report.rs  # exact-locale report presentation over immutable exports (merged #259; Active PR #421 presentation minimization)
...
├── postgres_longitudinal_observation.rs  # PostgreSQL immutable longitudinal observation/membership persistence (merged #248)
...
**Active PR #421** participant-facing result provenance minimization is not protected-main truth until its unchanged exact head is independently reviewed and gate-clean. It keeps all numeric scores, uncertainty, dispositions, limitations, owner/result/version/digest provenance, and server-authoritative export/render time in typed fields and the machine-readable result export while removing opaque internal identifiers and implementation metadata from localized/shared human-readable participant copy. Exact `ko-KR`/`en-US` localized copy explains the retained evidence in plain language. This PR does not alter psychometric arithmetic, authorization, tenant ownership, or immutable result evidence.
...
Merged #231 personal result export is protected-main domain evidence. `ResultExport::from_snapshot` copies the stored construct scores, standard errors, dispositions, owner `participant_ref`, and version provenance into JSON and its original human-readable report. Approved limitation text is required so the report cannot imply diagnosis, employment fitness, or a type score. Active PR #421 supersedes only that original human-readable internal-provenance presentation if merged; typed fields and machine-readable evidence remain exact. Padded export aliases are rejected at this boundary without rewriting shared reference trimming used by consent and other domains. The snapshot is not mutated. Do not fold unrelated persistence into this domain slice.
**Active PR** #257 authorized immutable personal result reads are not protected-main truth until an unchanged reviewed/check-clean head is integrated. `src/result_http.rs` accepts only exact `GET /v1/results/{result_ref}` targets, checks server-owned participant/result authorization before identity comparison, returns no result existence oracle to an unauthorized actor, and serializes the stored immutable provenance without recomputation. PostgreSQL result loading and cross-tenant HTTP E2E remain required before this becomes a complete product path.
...
Merged #231 personal result export is protected-main domain evidence. `ResultExport::from_snapshot` copies the stored construct scores, standard errors, dispositions, owner `participant_ref`, and version provenance into JSON and its original human-readable report. Active PR #421 changes only the human-readable presentation if merged: exact identity/version/digest/time evidence remains in typed fields and machine-readable export. Approved limitation text is required so the report cannot imply diagnosis, employment fitness, or a type score. Padded export aliases are rejected at this boundary without rewriting shared reference trimming used by consent and other domains. The snapshot is not mutated. HTTP export transport is already protected-main through merged #256. Do not fold unrelated persistence into this domain slice.

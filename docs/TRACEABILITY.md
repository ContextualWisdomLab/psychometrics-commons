...
| Immutable result provenance | PRD §3.1, §9.4 | TRD §9 | ADR-0004, ADR-0010 | **Implemented** in `src/result.rs`; **Active PR** #51 persists immutable snapshots; result-serving transport is Target |
...
| Durable outbox/inbox delivery semantics | PRD §7, §9 | TRD §19–20 | ADR-0014, ADR-0015 | **Partially implemented**: domain contracts in `src/integration.rs`; PostgreSQL 18 outbox/inbox identity plus delivery-attempt persistence; inbox consumption is Implemented; live side-effect execution remains Target |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001_integration_delivery.sql`, `migrations/0002_scoring_job_state.sql`, `migrations/0003_data_rights_propagation.sql`, `migrations/0006_instrument_release.sql`, `migrations/0011_scoring_request.sql`, and the matching PostgreSQL adapters, including expired-lease recovery and inbox consumption; **Active PR** #51 adds `migrations/0007_result_snapshot.sql` | remaining product aggregates, crash/restart restore acceptance |
...
Current protected-main Rust module surface on `e2ea529664f6f9ae6726ea8af96dfad6ae7c9f7e`:
...
**Active PR** #51 carries `migrations/0007_result_snapshot.sql`, the `postgres_result_snapshot` adapter, and real PostgreSQL tests for exact immutable snapshot/observation replay, fail-closed identity rebinding, and supersession without predecessor mutation. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Result-serving HTTP transport remains outside this slice.

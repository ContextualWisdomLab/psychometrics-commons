...
**Active PR** #233 scoring-worker claim-next (successor of #214, #203, and #200; composes #190/#172/#168) is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `claim_and_run_scoring_worker_attempt_with_result_snapshot` claims one due queued or retry-scheduled job, scores only the stored request pin, and drives `run_scoring_worker_attempt_with_result_snapshot`. A missing job row fails closed before the engine runs. A later due claim after a retryable outage persists the real snapshot and one stable terminal event. A claim before `next_attempt_at` stays `LeaseNotDue` with no snapshot or outbox. Prefer this head over #217, #214, #203, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those predecessors should not land separately. Live `fast-mlsirm` execution and scoring HTTP remain later adapters. Modules added on this stack and not yet protected-main truth include `src/scoring_worker.rs`, `src/postgres_scoring_worker.rs`, and `migrations/0007_result_snapshot.sql` (result-snapshot write already exists on protected main; worker composition does not).

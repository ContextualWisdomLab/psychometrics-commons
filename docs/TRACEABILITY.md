...
| English A1-B2 CEFR profile consumer | PRD §6.1, §9.11 | TRD §8.1; immutable external contract and result evidence binding | ADR-0021 | **Active PR** review-only product boundary in `src/cefr_language_assessment.rs` pins upstream Draft PR #5 commit/schema digests and rejects mismatched result identity; upstream release, schema-validator execution, instrument persistence, and result transport remain unverified/Target |

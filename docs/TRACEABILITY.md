...
| Scientific failure is typed, no invented score | TRD §8; Measurement Governance | **Active PR #402**: scoring contract tests plus `src/scoring_engine.rs` classify `ScientificScoringFailure` and request/result mismatch; `src/scoring_dispatch.rs` routes deterministic scientific/provenance failures to permanent quarantine while unclassified engine failures retain bounded retry | cross-process failure injection, protected-main integration, and live fast-mlsirm/provider evidence |

...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #160 adds hosted dual-proof write/recover commands in `src/account_link_write.rs`; **Active PR** #158 remains the store-wide restore-reconcile vehicle. Both include #147 persist.; HTTP/Keyverse token verification remain Target |

...
| Idempotent response events | PRD §9.2 | TRD §6 | ADR-0005, ADR-0010 | **Implemented** in `src/response.rs` with canonical SHA-256 payload-digest identity. **Active PR #427 / IMPLEMENTED_ON_ACTIVE_PR** adds PostgreSQL 18 accepted-event persistence, exact replay/conflict evidence, contiguous sequence recovery, and restart reconstruction; it is not protected-main truth until integrated |
...
| Conflicting idempotency replay fails closed | TRD §6 | `src/response.rs`; **Active PR #427 / IMPLEMENTED_ON_ACTIVE_PR** adds PostgreSQL uniqueness, exact replay/conflict classification, contiguous stored-prefix validation, and restart reconstruction | protected-main integration plus public response transport/E2E |
...
**Active PR #427 / IMPLEMENTED_ON_ACTIVE_PR** durable accepted response-event persistence is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `migrations/0020_response_event.sql` and `src/postgres_response_event.rs` keep the accepted mid-session ledger prefix durable across process restart with exact replay/conflict classification, contiguous positive sequence recovery, distinct observed/received clocks, immutable provenance, and fail-closed corrupt/gapped-history and reference contracts. Public response HTTP transport and completed snapshot reload remain separate slices. The normative logical ERD already contained `response_event`, its session cardinality, and its snapshot-entry relationship before this PR; #427 changes persistence maturity, not that logical entity shape, so `docs/architecture/ERD.md` requires no cardinality/entity remap for this slice.

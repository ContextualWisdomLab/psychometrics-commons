...
| Purpose-specific consent | PRD §5, §9.6 | TRD §12 | ADR-0006 | **Implemented** domain contract in `src/consent.rs`; grant/revoke/inspect are bound to participant ownership; **Active PR** #49 persists purpose-specific ledgers; HTTP transport remains Target |
...
| Durable outbox/inbox delivery semantics | PRD §7, §9 | TRD §19–20 | ADR-0014, ADR-0015 | **Partially implemented**: domain contracts in `src/integration.rs`; PostgreSQL 18 outbox/inbox identity plus delivery-attempt persistence; inbox consumption is **Implemented** on protected main; live side-effect execution remains Target |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001`–`0003`, `0006`, `0011`, `0012` and matching adapters; **Active PR** #49 adds `migrations/0005_consent_lifecycle.sql` | remaining product aggregates, crash/restart restore acceptance |
...
**Active PR** #49 carries `migrations/0005_consent_lifecycle.sql`, the `postgres_consent` adapter, and real PostgreSQL tests for exact ledger/event replay, research-revocation append, and fail-closed conflicting replay. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP consent transport remains outside this slice.

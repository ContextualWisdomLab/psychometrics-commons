...
|---|---|---|---|
| Server-authoritative session state | TRD §5 | `src/session.rs` + session contract tests, including published-release/locale binding at creation | **Active PR** #161 persist/load created-session identity and later command history with exact/conflicting replay, fail-closed stale shorter history, and header-row `FOR UPDATE`; **Active PR** #161 `POST /v1/sessions` mapping; live HTTP adapter remains missing |
...
| Only Published release accepts new sessions | TRD §7 | `PublicationState::accepts_new_sessions` in `src/instrument.rs`; `AssessmentSession` creation copies exact published release/version/locale provenance and fails closed on unpublished eligibility or locale mismatch | **Active PR** #161 persist and load created-session published-release/version/locale binding without re-checking current eligibility; **Active PR** #161 `src/session_http.rs` maps unpublished or locale-mismatched starts to RFC 9457 problems |
...
Still-Target logical modules/adapters include remaining product aggregate persistence/repositories, live HTTP adapters and remaining public/admin HTTP families, event transports, live fast-mlsirm/Keyverse/Gyeot/TEPP/semantic-data-portal adapters, research-release staging, deterministic narrative mapping, longitudinal normalized ingestion, participant identity-link history persistence, runtime health transports/metrics, and Measurement Workbench orchestration. Active work adds `src/postgres_assessment_session.rs`, `src/api_problem.rs`, `src/session_http.rs`, `contracts/openapi-public-v1.yaml`, `migrations/0014_assessment_session.sql`, and `migrations/0016_assessment_session_command.sql`; those files are not protected-main truth.

### Active implementation work that is not protected-main truth

**Active PR** #161 created-session persist, load, command-history replay, header-row lock, and `POST /v1/sessions` mapping is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Created sessions persist participant, published-release, version, digest, locale, state, and creation-time identity under `READ COMMITTED`; exact replay is idempotent and rebinding fails closed. Load restores that created identity without re-checking current publication eligibility. Later Activate/Pause/Resume commands persist in `assessment_session_command` and replay on load so an in-progress session survives restart. A stale persist that carries fewer commands than already stored fails closed and does not rewind the current-state projection. Command persist locks the `assessment_session` header with `SELECT … FOR UPDATE` before insert or count. `src/session_http.rs` plus `contracts/openapi-public-v1.yaml` map only `POST /v1/sessions` onto that created-session start and reviewed RFC 9457 problems. A live HTTP adapter and remaining public families remain outside this slice. Prefer this head over #146, #129, #125, #121, #109, persist-only #106, and #61. #87 is the RFC 9457 primitive predecessor.
...
**Active PR** #161 `contracts/openapi-public-v1.yaml` is the as-built OpenAPI 3.2.0 document for the implemented `POST /v1/sessions` mapping. It must not list unimplemented families. HTTP errors for that operation use RFC 9457 problem details. A live HTTP adapter remains Target. When additional HTTP operations are implemented, the same PR or a prerequisite PR must extend and validate this document so operations and problem responses match the actual implementation.

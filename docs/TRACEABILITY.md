...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #160 adds hosted dual-proof write/recover commands in `src/account_link_write.rs` and keeps a recovered participant only when the current tenant/issuer/subject still match the proof; **Active PR** #158 remains the store-wide restore-reconcile vehicle. Both include #147 persist.; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** successor of #160 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`; `src/account_link_write.rs` authorizes both proofs before persist, recovers from a still-valid account proof, and drops a loaded participant whose current binding no longer matches that proof | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |

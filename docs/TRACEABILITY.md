...
| Sequence-aware item delivery evidence | PRD §3.1, §9 | TRD §5–7 | ADR-0005, ADR-0010 | **Implemented** domain primitive in `src/item_delivery.rs`; **Active PR** #48 persists tenant-scoped item-delivery ledger and event evidence through `migrations/0004_item_delivery_evidence.sql`. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP delivery orchestration remains outside this slice.

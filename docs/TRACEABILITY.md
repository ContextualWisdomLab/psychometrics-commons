...
| Purpose-specific consent | PRD §5, §9.6 | TRD §12 | ADR-0006 | **Implemented** domain contract in `src/consent.rs` plus `migrations/0005_consent_lifecycle.sql` / `src/postgres_consent.rs` purpose-specific ledgers; HTTP transport remains Target. This Active PR reloads a persisted ledger after restart, reconstructing events by occurrence time, then `created_at`, then event identity, so a same-millisecond research revocation remains the latest decision |
...
**Active PR** consent-ledger reload is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `load_consent_ledger` reconstructs one participant ledger from `consent_ledger` / `consent_event` under `READ COMMITTED` after `FOR SHARE` on the header. Equal server timestamps keep insertion order. Out-of-order or unknown stored labels fail closed. This does not add outbox composition (see #120) or HTTP consent transport.

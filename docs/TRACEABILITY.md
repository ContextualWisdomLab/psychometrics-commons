...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #148 persists append-only `participant_identity_link` history, reloads it after restart, recovers the participant from unterminated history, reconciles a stale or missing current projection from unterminated history, and rejects a second unterminated subject in the database; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** identity-link successor `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, reconciles a stale or missing current projection from unterminated history, and rejects a second unterminated issuer-scoped subject in the database | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |
...
**Active PR** participant identity-link persistence is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Prefer this successor over #148, #147, #133, #124, and #114. `migrations/0022_participant_identity_link.sql`, `migrations/0023_participant_identity_link_unterminated_subject.sql`, and `src/postgres_participant_identity_link.rs` persist `assessment_participant`, append-only `participant_identity_link` / `participant_identity_link_end` evidence, and a derived `current_participant_identity_link` projection. Persist applies each link and then its matching ends so a complete unlink+relink aggregate can be written in one transaction. Unterminated issuer-scoped subjects are the lookup and uniqueness source of truth, so a returning account still recovers the same `participant_ref` when the derived projection is missing. Reconcile is the only current-row writer: it deletes tenant-scoped current rows whose links have ended, then restores or clears this participant's derived row so a stale projection cannot block relink or a later bind. A database trigger rejects a second unterminated issuer-scoped subject so lookup cannot become `CorruptHistory` after repair. HTTP account-link transport and live Keyverse token verification remain outside this slice. Migration `0021` remains reserved for #113 scoring-job health indexes.

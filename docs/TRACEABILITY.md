...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #147 persists append-only `participant_identity_link` history and adds hosted dual-proof write/recover commands in `src/account_link_write.rs`; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** successor of #147 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`; `src/account_link_write.rs` authorizes both proofs before persist and recovers from a still-valid account proof | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |

...
| Immutable response snapshot before scoring | PRD §9.3 | TRD §5–8 | ADR-0005, ADR-0010 | **Implemented** domain semantics in `src/response.rs` plus `migrations/0010_response_snapshot.sql` / `src/postgres_response_snapshot.rs` persist. Active PR #151 reloads that frozen prefix after restart so scoring can dispatch from the stored server order |

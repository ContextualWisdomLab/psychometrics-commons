...
**Active PR** #234 claim-next successor of #203 is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `claim_next_due_scoring_job` selects the oldest due queued or retry-scheduled row with `FOR UPDATE SKIP LOCKED`. `run_next_due_scoring_worker_attempt_with_result_snapshot` then calls only `run_scoring_worker_attempt_with_result_snapshot` using the stored `scoring_request_ref`, so a caller cannot complete job A with request B. A missing job row fails closed before the engine runs. A later due claim after a retryable outage persists the snapshot and exactly one terminal outbox row. Prefer this head over ready #203 and draft #217, #214, #207, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those siblings and predecessors should not land separately. Live `fast-mlsirm` execution and scoring HTTP remain later adapters. Modules added on this stack and not yet protected-main truth include `src/scoring_worker.rs`, `src/postgres_scoring_worker.rs`, and `migrations/0007_result_snapshot.sql` (result-snapshot write already exists on protected main; worker composition does not).

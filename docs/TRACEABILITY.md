...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #158 persists append-only `participant_identity_link` history, restores the derived current projection on exact replay, rebuilds that unique enforcer from unterminated history after restore, and inspects projection drift so operators run reconcile before new account-link writes; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** #158 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, inspects projection drift after restore, and rebuilds the derived unique enforcer before new account-link writes | HTTP unlink/relink transport and live Keyverse verification |

...
| Authorized immutable personal result read | PRD §3.1, §9.4 | TRD §18 `GET /v1/results/{result_ref}`; result authorization boundary | ADR-0003, ADR-0010 | **Implemented** merged #257 exposes a protected-main authorized immutable result read in `src/result_http.rs` that checks server-owned participant/result bindings before route identity; PostgreSQL resource loading and cross-tenant HTTP E2E remain Target |
...
| Quick and Deep assessment paths | PRD §3.1, §9 | TRD §5–7; immutable release/item-delivery boundary | ADR-0005, ADR-0010 | **Merged #261** ships protected-main release-bound Quick/Deep item subsets in `src/assessment_path.rs` bound to one immutable release with copied locale/provenance; path persistence, item-delivery transport, conversion, and scoring integration remain Target |
...
| EMA/ESM longitudinal flow | PRD §4 | TRD §16; UML longitudinal sequence; logical ERD extension | ADR-0008 | External Gyeot/TEPP dependencies + Target Commons enrollment/orchestration adapter; `src/longitudinal_observation.rs` records validity, recorded, received, and ingested clocks with explicit membership shares. Merged #248 ships protected-main PostgreSQL 18 persistence for immutable normalized observation records and membership shares via `migrations/0031_longitudinal_observation.sql` and `src/postgres_longitudinal_observation.rs`. **Active PR** #226 adds a consented enrollment domain contract in `src/longitudinal.rs` that gates Gyeot collection on the current consent ledger and tenant-owned participant record; enrollment persistence, HTTP, live Gyeot collection, and TEPP kernels remain Target |
...
| Authorized result read does not expose cross-tenant existence | TRD §11, §18; Security/Data | Protected-main `src/result_http.rs` (merged #257) authorizes server-owned participant/result records before comparing the requested reference and uses safe RFC 9457-style problem responses | PostgreSQL resource loading and cross-tenant HTTP E2E evidence |
...
├── postgres_data_rights_completion.rs  # PostgreSQL terminal data-rights completion evidence (merged #77)
...
├── postgres_longitudinal_observation.rs  # PostgreSQL immutable normalized longitudinal observation persistence (merged #248)
...
├── style_mapping.rs              # versioned deterministic Personality Style presentation mapping (ADR-0018)
...
├── result_http.rs  # authorized immutable personal result read transport (merged #257)
...
**Active PR** #226 longitudinal enrollment is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `src/longitudinal.rs` binds Gyeot program enrollment to a tenant-owned `ParticipantRecord` and an active longitudinal-observation consent snapshot, re-checks the current consent ledger and tenant-owned record before collection, preserves explicit multiple-membership contexts, and keeps pause/resume/withdraw fail-closed. The enroll-time `consent_snapshot_ref` is enrollment evidence only. Observation ingestion, enrollment persistence, and live Gyeot/TEPP adapters remain outside this slice. Do not merge #184/#199 heads for this goal; they authorize collection from enrollment state or snapshot-only state alone.
...
Closed-unmerged #301 consolidated public-release identity privacy gate is no longer an active lane; its fail-closed goal survives as open issue #260, which requires a fresh independently reviewed slice that fails closed when the authoritative restricted-identity inventory is absent or blank rather than being read as "nothing to match".
Merged #249 `authorize_result_export_read` is protected-main delivery-guard evidence in `src/result_export_authorization.rs`: it authorizes the stored participant/result with existing `ReadOwnResult` (ADR-0010 export provenance; ADR-0003 tenant-bound authorization) and then requires the export's `result_snapshot_ref` and copied `participant_ref` to match that exact immutable snapshot. Cross-tenant callers fail closed with the ordinary result-authorization denial before export-binding details are evaluated. No new permission or persistence was introduced.

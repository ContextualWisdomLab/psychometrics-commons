...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #52 carries `migrations/0008_participant_identity_link.sql`, the `postgres_identity_link` adapter, and real PostgreSQL tests for exact ledger/link/end replay plus fail-closed conflicting rebinding. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP identity-link transport remains outside this slice.

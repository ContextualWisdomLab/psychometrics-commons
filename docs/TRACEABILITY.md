...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #183 persists append-only `participant_identity_link` history, dual-proof persist/recover commands, and `POST /v1/account-links` / `POST /v1/account-links/recover` HTTP that uses the server clock; live Keyverse token verification remains Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** `src/postgres_participant_identity_link.rs` persists that history, `src/account_link_write.rs` authorizes both current proofs before persist/recover, and `src/account_link_http.rs` exposes persist/recover over HTTP | HTTP unlink transport, live Keyverse verification, and backup/restore evidence |
...
├── account_link_write.rs  # Active PR dual-proof persist/recover commands (not protected-main truth)
├── account_link_http.rs  # Active PR dual-proof persist/recover HTTP (not protected-main truth)
...
**Active PR** participant identity-link persistence, dual-proof write/recover, and hosted account-link HTTP are not protected-main truth until an unchanged reviewed/check-clean head is integrated. Prefer this successor over #183, #166, #160, #148, #147, #133, #124, and #114. `migrations/0022_participant_identity_link.sql`, `migrations/0023_participant_identity_link_unterminated_subject.sql`, and `src/postgres_participant_identity_link.rs` persist `assessment_participant`, append-only `participant_identity_link` / `participant_identity_link_end` evidence, and a derived `current_participant_identity_link` projection. Reconcile is the only current-row writer: it deletes tenant-scoped current rows whose links have ended, then restores or clears this participant's derived row so a stale projection cannot block relink or a later bind. `src/account_link_write.rs` authorizes both current proofs, persists that history, and recovers the same `participant_ref` from a still-valid account proof. `src/account_link_http.rs` and `openapi/account-links.yaml` expose `POST /v1/account-links` and `POST /v1/account-links/recover` with the server clock as link/recover time. A leftover current row after unlink does not make the write command tell the next buyer the account is taken. Live Keyverse token verification remains outside this slice. Migration `0021` remains reserved for #113 scoring-job health indexes.

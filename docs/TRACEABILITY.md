...
| Operation-scoped capability health | PRD §7, §13 | `docs/OPERABILITY.md` §3–4; Deployment/Operations | ADR-0011, ADR-0017 | **Implemented** domain health/readiness contract in `src/health.rs` plus `src/postgres_health.rs` PostgreSQL major/write-readiness and caller-declared relation presence; **Active PR** #82 observes aggregate outbox, inbox-consumption, and data-rights backlog counts/ages against caller-supplied policy only; HTTP probes, measured thresholds, and deployment-profile evidence remain Target |

...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #169 persists append-only `participant_identity_link` history, restores the derived current projection on exact replay, rebuilds that unique enforcer from unterminated history after restore, and inspects projection drift so operators run reconcile before new account-link writes; HTTP/Keyverse token verification remain Target |

...
| Export/deletion requires request-specific identity verification | TRD §13 | `src/data_rights.rs` plus persisted requested-state identity before later verification commands | Keyverse/account/anonymous transport integration |
...
├── postgres_data_rights.rs  # PostgreSQL requested-state plus dependent-system outbox persistence

...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** #124 persists append-only `participant_identity_link` history, reloads it after restart, and recovers the participant from the current issuer-scoped subject; HTTP/Keyverse token verification remain Target |

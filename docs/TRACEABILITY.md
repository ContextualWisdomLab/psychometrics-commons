...
| Scoring uses durable snapshot identity | TRD §8 | `src/scoring.rs` requires a canonical SHA-256 engine-artifact digest and protected-main `src/scoring_engine.rs` rejects a result that does not match the complete dispatched request provenance | live fast-mlsirm adapter + retry/outbox integration |
...
| Result export includes machine-readable provenance and the same scores | ADR-0010; PRD §3.1 | Protected-main `src/result_export.rs` copies immutable scores and provenance; authorized HTTP transport shipped through merged #256. **Active PR #421** retains exact owner/version/digest/time evidence in typed and machine-readable forms while privacy-minimizing only human-readable participant copy. | Active PR #421 exact-head checks/review and participant-copy regression evidence |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001_integration_delivery.sql`, `migrations/0002_scoring_job_state.sql`, `migrations/0003_data_rights_propagation.sql`, `migrations/0005_consent_lifecycle.sql`, `migrations/0006_instrument_release.sql`, `migrations/0011_scoring_request.sql`, `migrations/0012_integration_consumption.sql`, matching adapters, merged #77 `migrations/0024_data_rights_completion.sql`, and protected-main longitudinal observation persistence (`migrations/0031_longitudinal_observation.sql`) | remaining product aggregates, crash/restart restore acceptance |
...
**Active PR** #409 is the current-main landing vehicle for the public research-release identity privacy gate and supersedes stale/diverged #301. It requires an effective product-authorized restricted-identity inventory, rejects operational/credential/internal-location identities and restricted values fail-closed, preserves the separately governed research participant namespace, and leaves semantic-data-portal as the immutable public research catalog/release-registration owner. Historical #301 checks/reviews do not transfer.
...
Closed-unmerged #220 public research-fixture identity-column rejection is no longer an active lane; its fail-closed goal survives through the current-main privacy-gate work in #409 and its associated research-release contract lineage.

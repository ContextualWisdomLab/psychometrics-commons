...
**Active PR** #226 longitudinal enrollment is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `src/longitudinal.rs` binds Gyeot program enrollment to a tenant-owned `ParticipantRecord` and an active longitudinal-observation consent snapshot, re-checks the current consent ledger and tenant-owned record before collection, preserves explicit multiple-membership contexts, and keeps pause/resume/withdraw fail-closed. The enroll-time `consent_snapshot_ref` is enrollment evidence only. Observation ingestion, persistence, and live Gyeot/TEPP adapters remain outside this slice. Do not merge #184; that head authorized collection from enrollment state alone. Do not treat #199's snapshot-only collection gate as the landing vehicle.

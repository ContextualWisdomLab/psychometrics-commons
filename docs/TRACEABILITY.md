...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #166/#160 persists append-only `participant_identity_link` history, reconciles a stale current projection, and exposes dual-proof persist/recover commands; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** `src/postgres_participant_identity_link.rs` persists that history and `src/account_link_write.rs` authorizes both current proofs before persist/recover | HTTP unlink/relink transport, live Keyverse verification, and backup/restore evidence |

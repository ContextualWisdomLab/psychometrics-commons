...
|---|---|---|---|
| Server-authoritative session state | TRD §5 | `src/session.rs` + session contract tests, including published-release/locale binding at creation | **Active PR** #109 persist and load created-session identity with exact/conflicting replay; HTTP/API concurrency remains missing |

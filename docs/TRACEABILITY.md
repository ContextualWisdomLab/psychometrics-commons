...
| Optional Keyverse account linking | PRD §3.1, §9.7 | TRD §10; UML identity-link lifecycle | ADR-0003, ADR-0020 | **Partially implemented**: issuer-scoped first-link fail-closed domain primitive in `src/participant.rs`; **Active PR** successor of #192 persists append-only `participant_identity_link` history, restores the derived current projection on exact replay, inspects projection drift after restore, rebuilds that unique enforcer from unterminated history, frees an ended issuer-scoped subject for a later participant, adds dual-proof write/recover commands that refuse new links while inspect reports drift, and adds `persist_authorized_account_unlink` so a returning account can disconnect and later relink; HTTP/Keyverse token verification remain Target |
...
| Optional account linking does not rewrite historical participant/result identity | ADR-0003, ADR-0020 | `src/participant.rs` issuer-scoped first-link primitive preserves stable participant ID; **Active PR** successor of #192 `src/postgres_participant_identity_link.rs` persists and reloads that history without rewriting `participant_ref`, inspects projection drift after restore, rebuilds the derived unique enforcer, and frees an ended issuer-scoped subject for a later participant; `src/account_link_write.rs` authorizes both proofs, refuses new links until that inspect is clean, and persists an authorized unlink so recovery returns `None` until relink | HTTP unlink/relink transport and live Keyverse verification |

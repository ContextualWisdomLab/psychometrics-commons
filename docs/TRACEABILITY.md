...
| Operation-scoped capability health | PRD §7, §13 | `docs/OPERABILITY.md` §3–4; Deployment/Operations | ADR-0011, ADR-0017 | **Implemented** domain health/readiness contract in `src/health.rs` plus `src/postgres_health.rs` PostgreSQL major/write-readiness and caller-declared relation presence; **Active PR** #82 observes aggregate outbox, inbox-consumption, and data-rights backlog counts/ages and applies partial readiness indexes through `apply_backlog_health_index_migration`; the successor scoring-job slice observes queued/leased/retry-scheduled/quarantined job counts and created-at age through `apply_scoring_job_migration`; HTTP probes, measured thresholds, and deployment-profile evidence remain Target |
...
| Unknown/stalled backlog or unknown/incompatible integrity blocks new state-changing work | Operability §3, §6, §8 | **Implemented** domain contract in `src/health.rs`; `src/postgres_health.rs` fails closed on unsupported/read-only PostgreSQL or a missing required relation; **Active PR** #82 classifies durable integration and data-rights backlog evidence against an explicit caller policy; the successor scoring-job slice classifies queued/leased/retry-scheduled/quarantined job evidence the same way | HTTP probe wiring, measured deployment-profile thresholds, alerting, and failure-injection evidence |
...
**Active PR** #82 PostgreSQL operational-backlog observation is not protected-main truth until an unchanged reviewed/check-clean head is integrated. Aggregate outbox, inbox-consumption, and data-rights counts/ages are classified only against an explicit caller policy. Partial readiness indexes ship in `migrations/0020_backlog_health_indexes.sql` and must be applied through `apply_backlog_health_index_migration` after the owning table migrations; a private test `include_str!` is not a product apply path. The successor scoring-job slice adds the same content-free observation for queued, leased, retry-scheduled, and quarantined scoring work, with readiness indexes applied through `apply_scoring_job_migration`. HTTP probes and deployment-profile thresholds remain outside this slice.

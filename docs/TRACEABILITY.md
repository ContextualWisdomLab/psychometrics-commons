...
| Sequence-aware item delivery evidence | PRD §3.1, §9 | TRD §5–7 | ADR-0005, ADR-0010 | **Implemented** domain primitive in `src/item_delivery.rs`; **Active PR** #48 persists tenant-scoped ledger/event evidence; API delivery orchestration is Target |
...
| Initial physical persistence target is upstream PostgreSQL 18.x | ADR-0015; Deployment/Operations | **Implemented subset** in `migrations/0001`–`0003`, `0006`, `0011`, `0012` and matching adapters; **Active PR** #48 adds `migrations/0004_item_delivery_evidence.sql` | remaining product aggregates, crash/restart restore acceptance |
...
| Inbox receipt is not side-effect completion | ADR-0014/0015; UML integration sequence | `src/integration.rs` states/retry semantics; **Active PR** #58 persists pending/processing/completed consumption and expire-and-reclaim | live adapter crash/retry tests |
...
**Active PR** #48 persists tenant-scoped item-delivery ledger and event evidence through `migrations/0004_item_delivery_evidence.sql`. It is not protected-main truth until an unchanged reviewed/check-clean head is integrated. HTTP delivery orchestration remains outside this slice.

...
**Active PR** #214 scoring-worker request-bind plus retryable outage (successor of #203 and #200; composes #190/#172/#168) is not protected-main truth until an unchanged reviewed/check-clean head is integrated. `ScoringRequest::from_persisted` and `load_scoring_request` reconstruct the version pin after restart. `plan_scoring_worker_result_attempt` validates the caller envelope before asking a request-bound engine, then either binds one immutable `ResultSnapshot` or returns `ScoringWorkerResultPlan::Retryable` without an `event_ref`. `run_scoring_worker_attempt_with_result_snapshot` rejects a job/request mismatch before the engine runs. A completed result persists the snapshot, fenced job, and stable outbox event in one caller-owned transaction. A retryable engine or transport outage records the existing retry schedule, writes no snapshot and no terminal outbox row, and does not invent a score. A missing or corrupt request, planner failure, or snapshot conflict leaves the leased job untouched. Prefer this head over #203, #200, #190, #172, #168, #167, #155, #136, #115, and #69; those predecessors should not land separately. Live `fast-mlsirm` execution remains a later adapter behind the same engine trait. Modules added on this stack and not yet protected-main truth include `src/scoring_worker.rs`, `src/postgres_scoring_worker.rs`, and `migrations/0007_result_snapshot.sql` (result-snapshot write already exists on protected main; worker composition does not).

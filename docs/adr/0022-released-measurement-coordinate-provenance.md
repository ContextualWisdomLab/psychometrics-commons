...
## Target and as-built status

**Target:** Psychometrics Commons publishes an immutable, versioned measurement-coordinate authority that downstream longitudinal consumers can validate through a released ACL. TEPP binds separate predictor and outcome authorities without source copy or cross-service SQL.

**As built on protected main:** no such released coordinate contract exists. Protected `main@e81441ce70c676992470afe2be469dd891ad3eb5` owns immutable `ResultSnapshot` provenance but not this cross-repository projection. PR #449 is active Draft evidence only; its source, tests, ADR, and supplements are not protected-main truth and are not consumable release authority.
...
The v1 canonical envelope is bounded at **8,192 bytes**. This is a wire/resource-safety ceiling, not a scientific reference-length rule. Producer projection computes the encoded size before cloning snapshot provenance; consumer decode checks total byte length before UTF-8 parsing. Provenance is rejected rather than truncated. The limit is part of the v1 compatibility contract and changing it requires explicit compatibility review/versioning evidence.
...
## Dependency direction and consumer rule

Psychometrics Commons owns the measurement/result provenance projection. TEPP owns temporal admission, longitudinal composition, its artifact digest, and the consumer ACL. `fast-mlsirm` remains the reusable psychometric arithmetic owner.

Allowed direction is consumer → immutable released Psychometrics Commons contract. Forbidden coupling includes:

- copying `MeasurementCoordinateProvenance` source into TEPP;
- direct Psychometrics Commons database access from TEPP;
- making fast-mlsirm depend on Psychometrics Commons product code;
- treating a mutable branch/PR SHA as production contract authority.

TEPP#602 must bind **separate predictor and outcome authorities** into digest-bound longitudinal provenance. Missing, malformed, unsupported, mutable, or unreleased authority fails closed.

## Data, persistence, privacy and security

No new database table or migration is introduced by this decision. The projection is derived from immutable `ResultSnapshot` state and contains no participant/session/response/consent/narrative/source text. It carries opaque owner references and a canonical engine artifact digest only.

The producer must not truncate identity fields to satisfy the wire bound. Oversized projection fails closed. The decoder must reject oversized bytes before parsing/copying. Routine errors and logs may identify the typed failure class but must not log restricted source/participant content.

Because this is a participant-free provenance contract, CSAP/SOC 2 evidence concerns are integrity, release provenance, compatibility, least-privilege consumption and auditable change control rather than a new purpose for participant PII.

## Deployment, compatibility, rollback and recovery

This source projection has no independent runtime process and introduces no deployment topology or database migration. Compatibility is semantic-versioned through `MEASUREMENT_COORDINATE_CONTRACT_VERSION` plus strict decoder behavior. Unknown required semantics fail closed under QA-INT-03.

A mutable branch or PR head is development evidence only. Downstream repositories may treat this contract as production authority only after source lands on protected Psychometrics Commons `main` and an immutable release publishes the landed contract with exact-head CI/security/privacy/coverage/review, package/tag digest, SBOM/provenance, reproducibility, compatibility, and rollback evidence.

Rollback means consumers remain pinned to the previous supported immutable contract release; no in-place mutation of historical coordinate authority is permitted. If a released v1 defect requires incompatible repair, publish a new contract/release rather than silently changing canonical identity.

## Architecture-view impact

- **Bounded-context/component view:** adds an explicit released Measurement-provenance contract from Psychometrics Commons to longitudinal consumers such as TEPP.
- **UML/domain view:** `MeasurementCoordinateProvenance` is a projection/value contract derived from `ResultSnapshot`; it is not a new scoring aggregate or numerical estimator.
- **ERD/data view:** unchanged; no new persisted entity/table/relation is introduced.
- **Deployment view:** unchanged until a real transport/package consumption path is introduced.
- **Threat model:** adds malformed/oversized cross-repository provenance as a fail-closed input class; the 8 KiB bound and canonical decoder are the primary source controls.

Before this ADR can become Accepted, canonical architecture/TRACEABILITY/measurement-governance views affected by the released dependency direction must be reconciled ordinary-forward. A Draft supplement may record active-PR truth but does not replace protected-main canonical documentation.
...
## Evidence, follow-up and acceptance
...
Remaining follow-up before Accepted/release:

1. unchanged exact-head Runtime CI, rustdoc, line/branch/edge coverage, SAST, Security, CodeQL, SBOM and provenance evidence;
2. canonical TRACEABILITY and affected Measurement Governance / architecture-view reconciliation without truncating concurrent history;
3. qualifying independent current-head review;
4. normal protected-main merge;
5. immutable release with version/tag/package digest, SBOM/provenance, reproducibility, compatibility and rollback evidence;
6. TEPP#602 released-contract consumer RED → ACL repair → exact-head GREEN.

This ADR remains **Proposed** while those conditions are incomplete. It must not be marked Accepted merely because source tests or Draft documentation exist.

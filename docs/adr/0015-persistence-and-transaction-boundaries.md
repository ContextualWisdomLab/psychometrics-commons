...
A scoring worker that does not already know the next job identity claims the oldest due queued or retry-scheduled `scoring_job_state` row under `READ COMMITTED` with `FOR UPDATE SKIP LOCKED`, then records a terminal outcome only for that claimed job. The claim returns the stored `scoring_request_ref` and fencing lease, or `None` when no job is due. A scoring worker that records a terminal outcome must validate caller envelope fields, reconstruct the persisted `ScoringRequest` under `READ COMMITTED` after `FOR SHARE`, ask a request-bound engine, persist the immutable `ResultSnapshot`, then reuse one stable `event_ref` derived from the scoring job and the accepted result identity or permanent cause (Hohpe & Woolf, 2003; Richardson, 2018; PostgreSQL Global Development Group, 2026). The job row must name the same `scoring_request_ref` the caller supplies; a mismatched pair fails closed before the engine runs so a recovered claim cannot complete job A with request B. `ScoringWorkerEnvelope` has no caller-supplied event identity, so the planner binds the stable key after the engine returns and cannot accept a minted `event_ref`. A retryable engine or transport outage must call the existing retry schedule, release the lease, and write no terminal outbox row and no invented score. A later claim at or after the persisted due time must persist the real result snapshot and reuse one stable job-plus-result `event_ref`. Exhausted retry budget quarantines without an outbox row. A retry instant earlier than the outage, a missing or corrupt request, engine or planner failure, or snapshot conflict stays a distinct error, leaves the leased job untouched, and does not write outbox evidence. Live `fast-mlsirm` execution remains a later adapter behind that engine trait. Completion must occur inside the unexpired lease window.

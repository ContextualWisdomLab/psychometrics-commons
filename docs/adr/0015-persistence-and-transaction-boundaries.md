...
A scoring worker that records a terminal outcome must validate caller envelope fields, reconstruct the persisted `ScoringRequest` under `READ COMMITTED` after `FOR SHARE`, ask a request-bound engine, persist the immutable `ResultSnapshot`, then reuse one stable `event_ref` derived from the scoring job and the accepted result identity or permanent cause (Hohpe & Woolf, 2003; Richardson, 2018; PostgreSQL Global Development Group, 2026). Claim-next selects the oldest due queued or retry-scheduled row with `FOR UPDATE SKIP LOCKED`, reads `scoring_request_ref` from that job row, and calls only the bound snapshot worker so a recovered worker cannot name request B for job A (Hohpe & Woolf, 2003; PostgreSQL Global Development Group, 2026). The named-job helper still rejects a caller pin that does not match the stored row. `ScoringWorkerEnvelope` has no caller-supplied event identity, so the planner binds the stable key after the engine returns and cannot accept a minted `event_ref`. A retryable engine or transport outage must call the existing retry schedule, release the lease, and write no terminal outbox row and no invented score. A later claim-next at or after the persisted due time must persist the real result snapshot and reuse one stable job-plus-result `event_ref`. Exhausted retry budget quarantines without an outbox row. An empty or not-yet-due queue, a retry instant earlier than the outage, a missing or corrupt request, engine or planner failure, or snapshot conflict stays a distinct error and does not invent a score. Live `fast-mlsirm` execution remains a later adapter behind that engine trait.

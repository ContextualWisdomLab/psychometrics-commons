...
A scoring worker that records a terminal outcome must validate caller envelope fields, reconstruct the persisted `ScoringRequest` under `READ COMMITTED` after `FOR SHARE`, reject a job whose stored request is not the reconstructed pin, ask a request-bound engine, persist the immutable `ResultSnapshot`, then reuse one stable `event_ref` derived from the scoring job and the accepted result identity or permanent cause (Hohpe & Woolf, 2003; Richardson, 2018; PostgreSQL Global Development Group, 2026). `ScoringWorkerEnvelope` has no caller-supplied event identity, so the planner binds the stable key after the engine returns and cannot accept a minted `event_ref`. A retryable engine or transport outage records the existing job retry schedule in the same caller-owned transaction, writes no snapshot and no terminal outbox row, and does not invent a score. A hosted claim-next attempt must claim a due job and then call `run_scoring_worker_attempt_with_result_snapshot` using only the stored request pin; a missing job, a job that is not due, a pin mismatch, or a retry scheduled before the outage instant fails closed before a score can be invented. A missing or corrupt request, job/request mismatch, engine or planner failure, or snapshot conflict stays a distinct error, leaves the leased job untouched, and does not write outbox evidence. Live `fast-mlsirm` execution remains a later adapter behind that engine trait. Completion must occur inside the unexpired lease window.

...
- `IMPLEMENTED_ON_ACTIVE_PR`: PR #169 adds append-only `participant_identity_link` / `participant_identity_link_end` persistence, derived current-link projection, lifecycle-order persist of a complete unlink+relink aggregate, restart reload, current-subject lookup from unterminated history, exact-replay reconciliation of a missing or stale current projection, store-wide restore rebuild, a read-only drift inspect that tells operators to run reconcile before new account-link writes, and the inspect-then-reconcile path that frees an ended issuer-scoped subject for a later participant through `src/postgres_participant_identity_link.rs`. Prefer #169 over #173, #158, #147, #133, #124, and #114.
...
- Active-PR persistence evidence: PR #169 `migrations/0022_participant_identity_link.sql` and `src/postgres_participant_identity_link.rs` remain `IMPLEMENTED_ON_ACTIVE_PR` until merged. Prefer that inspect-and-reconcile head over #158, #147, #133, #124, and #114.

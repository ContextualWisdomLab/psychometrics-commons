...
- `IMPLEMENTED_ON_ACTIVE_PR`: PR #178 adds append-only `participant_identity_link` / `participant_identity_link_end` persistence, derived current-link projection, lifecycle-order persist of a complete unlink+relink aggregate, restart reload, current-subject lookup from unterminated history, exact-replay reconciliation of a missing or stale current projection, store-wide restore rebuild, a read-only drift inspect that tells operators to run reconcile before new account-link writes, and the inspect-then-reconcile path that frees an ended issuer-scoped subject for a later participant through `src/postgres_participant_identity_link.rs`. This successor adds `src/account_link_write.rs` so hosted adapters authorize both current proofs, refuse writes while inspect reports drift, persist the history, and recover the same `participant_ref` from a later valid account proof. Prefer #178 and this write-path successor over #173, #169, #160, #158, #147, #133, #124, and #114.
- `PLANNED`: durable HTTP transport, concurrency arbitration beyond the participant row lock, data-rights execution, full dump/restore drill evidence, and live Keyverse verification.
...
- Psychometrics Commons: add hosted HTTP unlink/relink/recovery transport with explicit idempotency, authority, and audit evidence.
...
- Active-PR persistence evidence: this write-path successor of PR #178 keeps `migrations/0022_participant_identity_link.sql` and `src/postgres_participant_identity_link.rs`, and adds `src/account_link_write.rs`. Prefer that inspect-then-write head over #178, #173, #169, #160, #158, #147, #133, #124, and #114.

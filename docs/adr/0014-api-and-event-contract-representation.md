...
- Supersedes: none
- Superseded by: none
- Current/as-built status: public/admin product HTTP transport and durable external event transport are not yet implemented on protected main; operator GET `/live` and GET `/ready` probes exist only on Active PR #117 (successor to #111) with `openapi/health-probes.yaml`; that same PR binds a TCP listener with bounded I/O timeouts, keeps serving after a dropped probe, answers `/live` without store I/O, and observes `observe_postgres_operational_snapshot` only for `/ready` without exposing driver errors

...
- Supersedes: none
- Superseded by: none
- Current/as-built status: public/admin product HTTP transport and durable external event transport are not yet implemented on protected main; operator GET `/live` and GET `/ready` probes exist only on Active PR #111 (successor to #91) with `openapi/health-probes.yaml`; that same PR binds a TCP listener with bounded I/O timeouts, can serve until accept fails, answers `/live` without store I/O, and observes `observe_postgres_operational_snapshot` only for `/ready` without exposing driver errors

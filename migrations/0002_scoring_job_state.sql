-- Scoring-job persistence accepts the same opaque-reference shape as the Rust domain.
...
DECLARE
    ...
    reference_contract_version CONSTANT TEXT :=
        'psychometrics-commons:scoring-job-reference-v2-default-ignorable';
    stored_reference_contract_version TEXT := obj_description(
        'scoring_job_reference_is_valid(text)'::regprocedure,
        'pg_proc'
    );
...
$create_scoring_job_state$;
    ...
    -- Validate the stored manifest before touching constraints. This preserves fail-closed drift
    -- detection while allowing a genuine previous migration state to be upgraded in place.
    SELECT ARRAY(
        SELECT format(
            '%s:%s:%s:%s:%s',
    ...
    reference_constraints_need_upgrade := reference_constraints_need_upgrade
        OR (
            NOT created_table
            AND stored_reference_contract_version IS DISTINCT FROM reference_contract_version
        );

    -- `CREATE TABLE` cannot update same-named CHECK constraints on an existing deployment, and
    -- `CREATE OR REPLACE FUNCTION` does not revalidate historical rows after predicate semantics
    -- change. Recreate the six reference guards when either a trusted previous manifest carries a
    -- legacy predicate or the function policy version advances. PostgreSQL validates each
    -- replacement against historical rows, so any old row that violates the stronger
    -- Rust-equivalent boundary makes the migration fail closed. An already-current schema keeps
    -- the same constraint objects and avoids repeated exclusive table locks plus full-table scans.
    ...
    EXECUTE format(
        'COMMENT ON FUNCTION scoring_job_reference_is_valid(TEXT) IS %L',

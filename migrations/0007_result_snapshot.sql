...
AS $result_snapshot_consent_refs$
    ...
    reference_character AS (
        ...
        CROSS JOIN LATERAL generate_series(1, character_length(reference_text)) AS character_index
    ),
    reference_classification AS (
        SELECT
            reference_text,
    ...
    SELECT
        count(*) = count(DISTINCT consent_reference.reference_text)
        AND COALESCE(
            bool_and(
                consent_reference.reference_text IS NOT NULL
                AND consent_reference.reference_text <> ''
                AND consent_reference.reference_text COLLATE "pg_unicode_fast" !~ '(^[[:space:]])|([[:space:]]$)'
                AND consent_reference.reference_text COLLATE "pg_unicode_fast" !~ '[[:cntrl:]]'
                AND NOT (classification.any_numeric AND classification.numeric_like_shape)
            ),
            TRUE
        )
    FROM consent_reference
    LEFT JOIN reference_classification AS classification USING (reference_text);

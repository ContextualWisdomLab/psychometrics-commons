...
--
-- Product reference identity is defined by Rust 1.97 `normalized_reference`: Unicode outer
-- whitespace is normalized, embedded controls are rejected, and numeric-like strings use
-- `char::is_numeric`, including Unicode No/Nl characters such as ½, ², and Ⅳ. PostgreSQL 18
-- UTF-8 with pg_unicode_fast supplies matching whitespace/control character semantics here.
...
CREATE TABLE IF NOT EXISTS data_rights_request_state (
    ...
    CONSTRAINT data_rights_request_ref_format_check CHECK (
        data_rights_reference_is_valid(request_ref)
    ),
    CONSTRAINT data_rights_tenant_ref_format_check CHECK (
        data_rights_reference_is_valid(tenant_ref)
    ),
    CONSTRAINT data_rights_participant_ref_format_check CHECK (
        data_rights_reference_is_valid(participant_ref)
    ),
    CONSTRAINT data_rights_scope_ref_format_check CHECK (
        data_rights_reference_is_valid(scope_ref)
    ),
...
CREATE TABLE IF NOT EXISTS data_rights_propagation_state (
    ...
    CONSTRAINT data_rights_dependent_system_ref_format_check CHECK (
        data_rights_reference_is_valid(dependent_system_ref)
    ),
    CONSTRAINT data_rights_propagation_source_ref_format_check CHECK (
        data_rights_reference_is_valid(source_ref)
    ),
    CONSTRAINT data_rights_propagation_event_ref_format_check CHECK (
        data_rights_reference_is_valid(event_ref)
    ),
...
-- CREATE TABLE IF NOT EXISTS preserves historical CHECK definitions. Recreate every reference
-- CHECK on apply so an upgrade validates all existing privacy evidence under the current Rust
-- identity predicate before it is considered healthy.
ALTER TABLE data_rights_propagation_state DROP CONSTRAINT IF EXISTS data_rights_propagation_event_ref_format_check;
ALTER TABLE data_rights_propagation_state DROP CONSTRAINT IF EXISTS data_rights_propagation_source_ref_format_check;
ALTER TABLE data_rights_propagation_state DROP CONSTRAINT IF EXISTS data_rights_dependent_system_ref_format_check;
ALTER TABLE data_rights_request_state DROP CONSTRAINT IF EXISTS data_rights_scope_ref_format_check;
ALTER TABLE data_rights_request_state DROP CONSTRAINT IF EXISTS data_rights_participant_ref_format_check;
ALTER TABLE data_rights_request_state DROP CONSTRAINT IF EXISTS data_rights_tenant_ref_format_check;
ALTER TABLE data_rights_request_state DROP CONSTRAINT IF EXISTS data_rights_request_ref_format_check;

ALTER TABLE data_rights_request_state ADD CONSTRAINT data_rights_request_ref_format_check CHECK (
    data_rights_reference_is_valid(request_ref)
);
ALTER TABLE data_rights_request_state ADD CONSTRAINT data_rights_tenant_ref_format_check CHECK (
    data_rights_reference_is_valid(tenant_ref)
);
ALTER TABLE data_rights_request_state ADD CONSTRAINT data_rights_participant_ref_format_check CHECK (
    data_rights_reference_is_valid(participant_ref)
);
ALTER TABLE data_rights_request_state ADD CONSTRAINT data_rights_scope_ref_format_check CHECK (
    data_rights_reference_is_valid(scope_ref)
);
ALTER TABLE data_rights_propagation_state ADD CONSTRAINT data_rights_dependent_system_ref_format_check CHECK (
    data_rights_reference_is_valid(dependent_system_ref)
);
ALTER TABLE data_rights_propagation_state ADD CONSTRAINT data_rights_propagation_source_ref_format_check CHECK (
    data_rights_reference_is_valid(source_ref)
);
ALTER TABLE data_rights_propagation_state ADD CONSTRAINT data_rights_propagation_event_ref_format_check CHECK (
    data_rights_reference_is_valid(event_ref)
);

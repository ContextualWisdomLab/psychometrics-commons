-- Response-snapshot persistence accepts the same opaque-reference shape as the Rust domain.
...
-- numeric and Default_Ignorable_Code_Point sets, while pg_unicode_fast supplies Unicode
...
AS $response_snapshot_reference$
...
CREATE TABLE IF NOT EXISTS response_snapshot (
    snapshot_ref TEXT CONSTRAINT response_snapshot_snapshot_ref_not_null NOT NULL
        CONSTRAINT response_snapshot_snapshot_ref_format_check CHECK (
            response_snapshot_reference_is_valid(snapshot_ref)
        ),
    session_ref TEXT CONSTRAINT response_snapshot_session_ref_not_null NOT NULL
        CONSTRAINT response_snapshot_session_ref_format_check CHECK (
            response_snapshot_reference_is_valid(session_ref)
        ),
...
CREATE TABLE IF NOT EXISTS response_snapshot_entry (
    ...
    event_ref TEXT CONSTRAINT response_snapshot_entry_event_ref_not_null NOT NULL
        CONSTRAINT response_snapshot_entry_event_ref_format_check CHECK (
            response_snapshot_reference_is_valid(event_ref)
        ),
    item_version_ref TEXT CONSTRAINT response_snapshot_entry_item_version_ref_not_null NOT NULL
        CONSTRAINT response_snapshot_entry_item_version_ref_format_check CHECK (
            response_snapshot_reference_is_valid(item_version_ref)
        ),
...
-- reference and digest constraints so existing product schemas are hardened as well as fresh ones.
...
ALTER TABLE response_snapshot_entry
    DROP CONSTRAINT IF EXISTS response_snapshot_entry_item_version_ref_format_check;
ALTER TABLE response_snapshot_entry
    DROP CONSTRAINT IF EXISTS response_snapshot_entry_event_ref_format_check;
ALTER TABLE response_snapshot
    DROP CONSTRAINT IF EXISTS response_snapshot_session_ref_format_check;
ALTER TABLE response_snapshot
    DROP CONSTRAINT IF EXISTS response_snapshot_snapshot_ref_format_check;
...
ALTER TABLE response_snapshot
    ADD CONSTRAINT response_snapshot_session_ref_format_check CHECK (
        response_snapshot_reference_is_valid(session_ref)
    );
ALTER TABLE response_snapshot_entry
    ADD CONSTRAINT response_snapshot_entry_event_ref_format_check CHECK (
        response_snapshot_reference_is_valid(event_ref)
    );
ALTER TABLE response_snapshot_entry
    ADD CONSTRAINT response_snapshot_entry_item_version_ref_format_check CHECK (
        response_snapshot_reference_is_valid(item_version_ref)
    );
ALTER TABLE response_snapshot_entry

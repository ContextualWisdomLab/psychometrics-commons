...
- Versioned cross-service integration envelopes plus bounded outbox retry/quarantine and source-scoped consumer inbox deduplication keyed by consumer, upstream source, and event identity, preserving exact replay idempotency, preventing cross-source event-reference collisions, failing closed on digest conflicts, and retaining safe audit evidence before physical persistence adapters are introduced.
